...
test_that("Reading of set works", {
  expect_equal(gdxio$rgdx(file.path(getwd(), "dätä/test_gdxio.gdx"), "i"), 
               tibble::tibble('1' = c("seattle", "san-diego"), '2' = rep.int(NA_character_, 2L)))
})
test_that("Reading of parameter works", {
  expect_identical(gdxio$rgdx(file.path(getwd(), "dätä/test_gdxio.gdx"), "a"), 
                   tibble::tibble('1' = c("seattle", "san-diego"), '2' = c(350, 600)))
})
test_that("Reading of table with squeezed out column works", {
  expect_identical(gdxio$rgdx(file.path(getwd(), "dätä/test_gdxio.gdx"), "squeezed_out"), 
  ...
  filePath <- file.path(getwd(), "dätä/tests_gdxio.gdx")
  on.exit(unlink(filePath), add = TRUE)
  gdxio$wgdx(filePath, data)
  expect_identical(gdxio$rgdx(file.path(getwd(), "dätä/tests_gdxio.gdx"), "squeezed_out"), 
                   tibble::tibble('1' = c("seattle", "san-diego"), 'asd' = c(0, 0),
                                  'def' = c(1, 2)))
})
test_that("Reading of (single) scalar works", {
  expect_identical(gdxio$rgdx(file.path(getwd(), "dätä/test_gdxio.gdx"), "f"), 
                  90)
})
test_that("Reading of (single) singleton set works", {
  expect_identical(gdxio$rgdx(file.path(getwd(), "dätä/test_gdxio.gdx"), "sub_i"), 
  ...
  on.exit(unlink(filePath), add = TRUE)
  gdxio$wgdx(filePath, data)
  expect_equal(gdxio$rgdx(file.path(getwd(), "dätä/tests_gdxio.gdx"), "sub_i"), 
...
test_that("Reading of input scalars works", {
  expect_equal(gdxio$rgdx(file.path(getwd(), "dätä/test_gdxio.gdx"), scalarsFileName), 
...
test_that("Reading of output scalars works", {
  expect_identical(gdxio$rgdx(file.path(getwd(), "dätä/test_gdxio.gdx"), scalarsOutName), 
...
test_that("Reading of (single) scalar equation works", {
  expect_identical(gdxio$rgdx(file.path(getwd(), "dätä/test_gdxio.gdx"), "cost"), 
                   c(0, 1, 0, 0, 1))
})
test_that("Reading of (single) scalar variable works", {
  expect_identical(gdxio$rgdx(file.path(getwd(), "dätä/test_gdxio.gdx"), "z"), 
                   c(153.675, 0, -Inf, Inf, 1))
})
test_that("Reading of scalar variables and equations works", {
  expect_identical(gdxio$rgdx(file.path(getwd(), "dätä/test_gdxio.gdx"), scalarEquationsOutName), 
...
test_that("Reading of equations works", {
  expect_identical(gdxio$rgdx(file.path(getwd(), "dätä/test_gdxio.gdx"), 'supply'), 
...
test_that("Reading of variables works", {
  expect_equal(as.data.frame(gdxio$rgdx(file.path(getwd(), "dätä/test_gdxio.gdx"), 'x')), 
...
test_that("Writing of variables works", {
  ...
  on.exit(unlink(filePath), add = TRUE)
  gdxio$wgdx(filePath, data)
  expect_equal(gdxio$rgdx(file.path(getwd(), "dätä/tests_gdxio.gdx"), 'x'), 
               varData)
})
...
test_that("Writing of equations works", {
  ...
  data <- list(eqData)
  names(data) <- "supply"
  filePath <- file.path(getwd(), "dätä/tests_gdxioeq.gdx")
  on.exit(unlink(filePath), add = TRUE)
  gdxio$wgdx(filePath, data)
  expect_identical(gdxio$rgdx(file.path(getwd(), "dätä/tests_gdxioeq.gdx"), "supply"), 
                   eqData)
})
...
test_that("Reading/writing unicode characters work", {
  ...
  data <- list(setData)
  names(data) <- "i"
  filePath <- file.path(getwd(), "dätä/tests_gdxiouni.gdx")
  on.exit(unlink(filePath), add = TRUE)
  gdxio$wgdx(filePath, data)
  expect_identical(gdxio$rgdx(file.path(getwd(), "dätä/tests_gdxiouni.gdx"), "i"), 
                   setData)
})
