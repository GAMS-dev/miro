let gmsSymIn=new Object();let gmsSymOut=new Object();let gmsSymHdrIn=new Object();let gmsSymNumHdrIn=new Object();let gmsSymHdr=new Object();let scalarSyms=[];let scalars=[];function parseGMSIO(){$("#errMsg").innerHTML="";var files=$("#selectFile")[0].files;if(files.length<=0){return!1}
var fr=new FileReader();fr.onload=function(e){try{var gmsData=JSON.parse(e.target.result);gmsSymIn=Object.keys(gmsData.gamsInputFiles);gmsSymOut=Object.keys(gmsData.gamsOutputFiles);var gmsSymHdrOut=new Object();var gmsSymNumHdrOut=new Object();var i=0;while(i<gmsSymIn.length){if($.inArray("headers",Object.keys(gmsData.gamsInputFiles[gmsSymIn[i]]))===-1||gmsSymIn[i]==="scalars"){if(gmsSymIn[i]==="scalars"){scalars.push("scalars");gmsSymHdrIn[gmsSymIn[i]]=Object.keys(gmsData.gamsInputFiles[gmsSymIn[i]].headers);gmsSymNumHdrIn[gmsSymIn[i]]=["Value"];scalarSyms=scalarSyms.concat(gmsData.gamsInputFiles[gmsSymIn[i]].symnames)}else{scalarSyms.push(gmsSymIn[i])}
...
for(let i in gmsSymOut){if(gmsSymOut[i]==="scalars_out"){scalars.push("scalars_out");gmsSymNumHdrOut[gmsSymOut[i]]=["Value"]}else{gmsSymNumHdrOut[gmsSymOut[i]]=[]}
...
if(typeof co.dataRendering["Format: graph"]!=='undefined'){for(var i=0;i<co.dataRendering["Format: graph"].length;i++){var grname=co.dataRendering["Format: graph"][i].Parameter;co.dataRendering[grname]=co.dataRendering["Format: graph"][i];delete co.dataRendering[grname].Parameter;if(co.dataRendering[grname].outType==="single view"){co.dataRendering[grname].outType="graph"}
...
if(co.dataRendering[grname].graph.choice==="UseDygraphs"){co.dataRendering[grname].graph.tool="dyGraphs";$.extend(co.dataRendering[grname].graph,co.dataRendering[grname].graph.dygraphs);delete co.dataRendering[grname].graph.dygraphs;for(var k=0;k<co.dataRendering[grname].graph.dyEvent.length;k++){var eventname=co.dataRendering[grname].graph.dyEvent[k].eventdata;delete co.dataRendering[grname].graph.dyEvent[k].eventdata;co.dataRendering[grname].graph.dyEvent[eventname]=co.dataRendering[grname].graph.dyEvent[k];delete co.dataRendering[grname].graph.dyEvent[k]}
...
delete co.dataRendering[grname].graph.choice;if((co.dataRendering["Format: graph"][i].graph.choice==="UseDygraphs")||((co.dataRendering["Format: graph"][i].graph.choice==="UsePlotly")&&(co.dataRendering[grname].graph.plotly.graphtype.choice!=="Histogram"))){for(var j=0;j<co.dataRendering[grname].graph.ydata.length;j++){var yname=co.dataRendering[grname].graph.ydata[j].dataname;delete co.dataRendering[grname].graph.ydata[j].dataname;co.dataRendering[grname].graph.ydatatmp[yname]=co.dataRendering[grname].graph.ydata[j]}
...
let widgetKeys=['inputWidgets','inputWidgetsCL'];for(let widgetKeyId=0;widgetKeyId<widgetKeys.length;widgetKeyId++){let widgetKey=widgetKeys[widgetKeyId];if(typeof co[widgetKey]!=='undefined'){let inputWidgets=new Object();for(let wId=0;wId<co[widgetKey].length;wId++){let widget=co[widgetKey][wId];let gmsParam=null;if(typeof widget.paramType!=='undefined'){if(widget.paramType==="Gams option"){gmsParam="GMSOPT_"+widget.gmsParam}else{gmsParam="GMSPAR_"+widget.gmsParam}
delete widget.paramType}else{gmsParam=widget.gmsParam}
delete widget.gmsParam;switch(widget.widgetType){case "slider":function addOp(opName,sliderVal){let sliderValOp;switch(opName){case "count":sliderValOp="card("+sliderVal+")";break;case "maximum":sliderValOp="max("+sliderVal+")";break;case "mean":sliderValOp="mean("+sliderVal+")";break;case "median":sliderValOp="median("+sliderVal+")";break;case "minimum":sliderValOp="min("+sliderVal+")";break;case "standard deviation":sliderValOp="sd("+sliderVal+")";break;case "variance":sliderValOp="var("+sliderVal+")";break}
return sliderValOp}
if(widget.slider.minDepSel==="dependent"){let sliderVal="";if($.inArray("minpar",Object.keys(widget.slider.minDep))!==-1){sliderVal=widget.slider.minDep.minpar+"$"}
sliderVal=sliderVal+widget.slider.minDep.minhdr;sliderVal=addOp(widget.slider.minDep.minop,sliderVal);widget.slider.min=sliderVal;delete widget.slider.minDep}
delete widget.slider.minDepSel;if(widget.slider.maxDepSel==="dependent"){let sliderVal="";if($.inArray("maxpar",Object.keys(widget.slider.maxDep))!==-1){sliderVal=widget.slider.maxDep.maxpar+"$"}
sliderVal=sliderVal+widget.slider.maxDep.maxhdr;sliderVal=addOp(widget.slider.maxDep.maxop,sliderVal);widget.slider.max=sliderVal;delete widget.slider.maxDep}
delete widget.slider.maxDepSel;if(widget.slider.defDepSel==="dependent"){let sliderVal="";if($.inArray("defpar",Object.keys(widget.slider.defDep))!==-1){sliderVal=widget.slider.defDep.defpar+"$"}
sliderVal=sliderVal+widget.slider.defDep.defhdr;sliderVal=addOp(widget.slider.defDep.defop,sliderVal);widget.slider.default=sliderVal;delete widget.slider.defDep}
delete widget.slider.defDepSel;if(widget.slider.slidertype==="slider range"){widget.slider.default=[widget.slider.defaultmin,widget.slider.defaultmax];delete widget.slider.defaultmin;delete widget.slider.defaultmax}
delete widget.slider.slidertype;if(widget.slider.ticks){delete widget.slider.ticks}
if(!widget.slider.noHcube){delete widget.slider.noHcube}
if(!widget.slider.noImport){delete widget.slider.noImport}
if($.isEmptyObject(widget.slider)){delete widget.slider}else{widget.slider.alias=widget.alias;widget.slider.widgetType=widget.widgetType;widget=widget.slider}
break;case "dropdown":if(widget.dropdown.aliases.length!==0&&widget.dropdown.choices.length!==widget.dropdown.aliases.length){alert("The number of choices does not match the number of aliases! Please make sure you enter as many aliases as you entered choices.");return}
let choicesR=widget.dropdown.choices;let choices=[];for(let i=0;i<choicesR.length;i++){if(choicesR[i].choiceDepSel==="dependent"){let choice="";if($.inArray("choicepar",Object.keys(choicesR[i].choiceDep))!==-1){choice=choicesR[i].choiceDep.choicepar+"$"}
...
if(!widget.dropdown.choiceMandatory){choices.push("_")}
widget.dropdown.choices=choices;choicesR=widget.dropdown.aliases;let aliases=[];for(let i=0;i<choicesR.length;i++){if(choicesR[i].aliasDepSel==="dependent"){let alias="";if($.inArray("aliaspar",Object.keys(choicesR[i].aliasDep))!==-1){alias=choicesR[i].aliasDep.aliaspar+"$"}
...
if(aliases.length){if(!widget.dropdown.choiceMandatory){aliases.push("_")}
widget.dropdown.aliases=aliases}else{delete widget.dropdown.aliases}
delete widget.dropdown.choiceMandatory
if(!widget.dropdown.noHcube){delete widget.dropdown.noHcube}
if(!widget.dropdown.noImport){delete widget.dropdown.noImport}
if($.isEmptyObject(widget.dropdown)){delete widget.dropdown}else{widget.dropdown.alias=widget.alias;widget.dropdown.widgetType=widget.widgetType;widget=widget.dropdown}
break;case "date selector":if(widget.date.format==="yyyy-mm-dd"){delete widget.date.format}
if(widget.date.startview==="month"){delete widget.date.startview}
if(!widget.date.noHcube){delete widget.date.noHcube}
if(!widget.date.noImport){delete widget.date.noImport}
if($.isEmptyObject(widget.date)){delete widget.date}else{widget.date.alias=widget.alias;widget.date.widgetType=widget.widgetType;widget=widget.date}
break;case "date range selector":if(widget.daterange.format==="yyyy-mm-dd"){delete widget.daterange.format}
if(widget.daterange.startview==="month"){delete widget.daterange.startview}
if(widget.daterange.weekstart===0){delete widget.daterange.weekstart}
if(widget.daterange.separator===" to "){delete widget.daterange.separator}
if(widget.daterange.autoclose===!1){delete widget.daterange.autoclose}
if(!widget.daterange.noHcube){delete widget.daterange.noHcube}
if(!widget.daterange.noImport){delete widget.daterange.noImport}
if($.isEmptyObject(widget.daterange)){delete widget.daterange}else{widget.daterange.alias=widget.alias;widget.daterange.widgetType=widget.widgetType;widget=widget.daterange}
break;case "checkbox":if(widget.checkbox.value){widget.checkbox.value=1}else{widget.checkbox.value=0}
if(!widget.checkbox.noHcube){delete widget.checkbox.noHcube}
if(!widget.checkbox.noImport){delete widget.checkbox.noImport}
if($.isEmptyObject(widget.checkbox)){delete widget.checkbox}else{widget.checkbox.alias=widget.alias;widget.checkbox.widgetType=widget.widgetType;widget=widget.checkbox}
break;default:if(!widget.table.readOnlyCols.length){delete widget.table.readOnlyCols}
if(!widget.table.noImport){delete widget.table.noImport}
if(!widget.table.readonly){delete widget.table.readonly}
if($.isEmptyObject(widget.table)){delete widget.table}else{widget.table.alias=widget.alias;widget.table.widgetType='table';widget=widget.table}}
if(!$.isEmptyObject(widget)){inputWidgets[gmsParam]=widget}}
if(widgetKeyId===0){co.inputWidgets=inputWidgets}else{if(typeof co.inputWidgets==='undefined'){co.inputWidgets=inputWidgets}else{$.extend(co.inputWidgets,inputWidgets)}
delete co.inputWidgetsCL}}}
if(existingConfig!==null){$.extend(co,existingConfig)}
let data="text/json;charset=utf-8,"+encodeURIComponent(JSON.stringify(co,null,"  "));$("<a href=\"data:"+data+"\" download=\"config.json\" id=\"jsonDlLink\">Download</a>").appendTo("body");$("#jsonDlLink")[0].click();$("#jsonDlLink").remove()}}}}},"postRender":function(control){var pivot=control.getControlByPath("dataRendering/Format: pivottable");pivot.on("change",function(){for(let i=0;i<pivot.children.length;i++){let gmsSymName=pivot.children[i].childrenByPropertyId.Parameter.data[0].text;let pivottable=pivot.children[i].childrenByPropertyId.pivottable
