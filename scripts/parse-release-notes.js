...
const BASE = process.argv[4] || process.env.MIRO_BASE || "https://gams.com/miro/";
...
function absolutizeAnchors($, root, base) {
  root.find("a[href]").each((_, a) => {
    ...
    const isAbsolute = /^(?:[a-z]+:)?\/\//i.test(href) || /^[a-z]+:/i.test(href);
...
function extractReleaseItems($, rootList, base) {
  ...
  rootList.children("li").each((_, li) => {
    ...
    if (childLists.length) {
      const labelHTML = htmlWithoutNestedLists($, $li, base).replace(/\s*:\s*$/, "");
      childLists.each((_, subList) => {
        $(subList)
          .children("li")
          .each((__, subLi) => {
            const subHTML = htmlWithoutNestedLists($, subLi, base);
            if (subHTML) items.push(labelHTML ? `${labelHTML}: ${subHTML}` : subHTML);
          });
      });
    } else {
...
latest.el.find("h3").each((_, h3) => {
  ...
  if (heading === "new features" || heading === "minor new features and improvements") {
