...
mkdir -p r
if [[ $(uname -p) == 'arm' ]]; then
    curl -o r/r.pkg https://cloud.r-project.org/bin/macosx/sonoma-arm64/base/R-${R_BASE_VERSION}-arm64.pkg
else
...
pushd r > /dev/null
    ...
    rm -rf bin/R.bak SVN-REVISION doc tests lib/*.dSYM Info.plist R.framework library/foreign/files/._sids.dbf
