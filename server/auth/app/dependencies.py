...
def get_user_groups(auth_header: str, is_admin: bool) -> list[str]:
    try:
        ...
        logger.exception(
            "Internal error when requesting user groups for namespace: %s.",
            settings.engine_ns,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
def get_authenticated_user(bearer_token: str, username: str) -> User:
    try:
        ...
        if namespace_permissions == 0:
            logger.info(
                "User '%s' has no permissions on namespace: %s",
                username,
                settings.engine_ns,
            )
        ...
        logger.info(
            "User '%s' successfully authenticated on namespace: %s (permissions: %s)",
            username,
            settings.engine_ns,
            str(namespace_permissions),
        )
        return User(
            name=username,
        ...
        logger.exception(
            "Internal error when requesting permissions for namespace: %s.",
            settings.engine_ns,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_current_app_user(
    ...
    credentials_bearer: HTTPBasicCredentials = Depends(bearer_auth),
    app_id: str = Path(
        description="The ID of the app whose data you want to operate on.",
        max_length=60,
    ),
) -> User:
...
async def get_current_admin_user(
    ...
    try:
        ...
        if not is_admin:
            logger.info(
                "User '%s' does not have full permissions (is not MIRO admin) on namespace: %s",
                username,
                settings.engine_ns,
            )
            raise HTTPException(
        ...
        logger.exception("Internal error when requesting user info from GAMS Engine")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Internal server error",
        ) from exc
