...
@router.get(
    "/", summary=metadata["summary"]["get"], response_model=list[ScenarioConfig]
)
async def get_scenario_list(
    ...
    try:
        ...
        logger.info(
            "Problems adding new MIRO scenario. Status code: %s. Details: %s",
            exc.status_code,
            exc.detail,
        )
        raise exc
    except Exception as exc:
        logger.info("Problems adding new MIRO scenario. Details: %s", str(exc))
        raise HTTPException(
...
@router.delete(
    "/",
    summary=metadata["summary"]["delete"],
    responses={
...
async def delete_scenario(
    ...
    user: Annotated[User, Depends(get_current_app_user)],
    name: Annotated[
        str, Query(description=metadata["description"]["name"], max_length=63)
    ],
    owner: Annotated[
        str | None, Query(description=metadata["description"]["owner"], max_length=70)
    ] = None,
):
    """
    ...
    """
    logger.info(
        "%s requested to delete scenario: %s of app: %s", user.name, name, app_id
    )
    try:
        await delete_data(user, app_id, name, owner)
    except HTTPException as exc:
        logger.info(
            "Problems deleting MIRO scenario. Status code: %s. Details: %s",
            exc.status_code,
            exc.detail,
        )
        raise exc
    except Exception as exc:
        logger.info("Problems deleting MIRO scenario. Details: %s", str(exc))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Internal Server Error",
        ) from exc
...
@router.get(
    "/download",
    summary=metadata["summary"]["download"],
    responses={422: {"description": "Duplicate records found when writing GDX file"}},
)
async def download_scenario(
    ...
    file_type: Annotated[
        ExportFileType, Query(description=metadata["description"]["file_type"])
    ] = ExportFileType.MIROSCEN,
    owner: Annotated[
        str | None, Query(description=metadata["description"]["owner"], max_length=70)
    ] = None,
):
    ...
    """
    logger.info(
        "%s requested to download scenario: %s of app: %s", user.name, name, app_id
    )
    ...
    try:
        ...
        logger.info(
            "%s: Scenario: %s of app: %s successfully downloaded",
            user.name,
            name,
            app_id,
        )
        ...
        cleanup()
        logger.info(
            "Problems downloading MIRO scenario. Status code: %s. Details: %s",
            exc.status_code,
            exc.detail,
        )
        raise exc
        ...
        cleanup()
        logger.info("Problems downloading MIRO scenario. Details: %s", str(exc))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Internal Server Error",
        ) from exc
...
async def add_scenario(
    ...
    scenario_data: Annotated[
        UploadFile, File(description=metadata["description"]["scenario_data"])
    ],
    overwrite_data: Annotated[
        bool, Form(description=metadata["description"]["overwrite_data"])
    ] = False,
    read_perm: Annotated[
        list[str], Form(description=metadata["description"]["read_perm"])
    ] = None,
    write_perm: Annotated[
        list[str], Form(description=metadata["description"]["write_perm"])
    ] = None,
    exec_perm: Annotated[
        list[str], Form(description=metadata["description"]["exec_perm"])
    ] = None,
):
    """
    Add a new scenario.
    ...
    try:
        await add_data(
            user, app_id, scenario_data, permissions, overwrite_data=overwrite_data
        )
    except HTTPException as exc:
        logger.info(
            "Problems adding new MIRO scenario. Status code: %s. Details: %s",
