...
@router.post(
    "",
    summary="Log in user and return login info",
...
async def login(auth_request: AuthRequest):
    ...
    logger.info(
        ...
        auth_request.username,
        str(authenticated_user.is_admin),
    )
    return AuthResponse(
        token=bearer_token,
...
@router.post(
    "/oidc",
    summary="Log in user via OpenID Connect and return login info",
    status_code=status.HTTP_200_OK,
...
async def login_oidc(auth_request: OidcAuthRequest):
    ...
    logger.info(
        "User: %s successfully logged in via OIDC (is_admin: %s).",
        user_info.username,
        str(authenticated_user.is_admin),
    )
