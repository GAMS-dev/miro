...
async def update_app(
    app_data: Annotated[
        UploadFile, File(description=metadata["description"]["app_data"])
    ],
    ...
    """
    logger.info("%s requested to update existing app: %s", admin_user.name, app_id)
    if access_groups:
    ...
    try:
        await add_or_update_app(
            admin_user, app_config, app_data, overwrite_data=overwrite_data, update=True
        )
    ...
    logger.info("App with id: %s successfully updated by: %s.", app_id, admin_user.name)
...
async def add_app(
    ...
    """
    logger.info("%s requested to add new app", admin_user.name)
    if access_groups:
        nonempty_access_groups = [group for group in access_groups if group != ""]
    else:
        nonempty_access_groups = []
    ...
    try:
        await add_or_update_app(
            admin_user, app_config, app_data, overwrite_data=overwrite_data
        )
    except HTTPException as exc:
        logger.info(
            "Problems adding new MIRO app. Status code: %s. Details: %s",
            exc.status_code,
            exc.detail,
        )
        raise exc
    except Exception as exc:
        logger.info("Problems adding new MIRO app. Details: %s", str(exc))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Internal Server Error",
        ) from exc
    logger.info("New app with successfully added by: %s.", admin_user.name)
...
async def delete_app(
    app_id: Annotated[
        str, Path(description="The ID of the app to delete.", max_length=60)
    ],
    admin_user: Annotated[User, Depends(get_current_admin_user)],
    delete_data: Annotated[
        bool, Query(description="Whether to delete all scenario data of this app.")
    ] = False,
):
    """
    ...
    logger.info("App with id: %s successfully deleted by: %s.", app_id, admin_user.name)
