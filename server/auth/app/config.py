...
with open("/home/miro/admin/global.R", "r", encoding="utf-8") as f:
    MIRO_SERVER_VERSION = re.search(
        r'MIRO_VERSION\s*<-\s*"(\d+\.\d+\.\d+)"\s*', f.read()
    ).group(1)

FORCE_SIGNED_APPS = (
    "force-signed-apps" in settings_yml and settings_yml["force-signed-apps"] == True
)
FORCE_SIGNED_APPS = "true" if FORCE_SIGNED_APPS else "false"

OIDC_LOGIN = (
    "authentication" in settings_yml and settings_yml["authentication"] == "openid"
)
...
class Settings(BaseSettings):
    ...
    authentication_mode: str = Field("oidc" if OIDC_LOGIN else "engine")
