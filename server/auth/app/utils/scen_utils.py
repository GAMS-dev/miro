        ...
        try:
            ...
            if exc.status_code == 418:
                ...
                raise HTTPException(
                    ...
                    detail="A scenario with this name already exists") from exc
            raise HTTPException(
                status_code=exc.status_code, detail=exc.detail) from exc
...
async def download_data(user_info: User, app_id: str, file_path: str,
                        file_type: ExportFileType, scen_name: str, scen_owner: str) -> None:
    proc_input = json.dumps({
        "mode": "download",
        "appId": app_id,
