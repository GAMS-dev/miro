...
class TestEngine:
    def test_login_invalid(self, cleanup):
        ...
        response = client.post(
            "/login", json={"username": settings["ENGINE_USER"], "password": "invalid"}
        )
        print(response.json())
        assert response.status_code == 401
        response = client.post("/login", json={"username": settings["ENGINE_USER"]})
        print(response.json())
    ...
    def test_login_no_perm(self, cleanup):
        ...
        response = client.post(
            "/login",
            json={"username": "mirotests_auth_1", "password": "mirotests_auth_11234"},
        )
        print(response.json())
        assert response.status_code == 403
        response = requests.post(
            f"{settings['ENGINE_URL']}/namespaces/{settings['ENGINE_NS']}/user-groups/mygroup",
