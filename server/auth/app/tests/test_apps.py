...
class TestApps:
    ...
    def test_post_app(self, cleanup):
        ...
        response = client.get("/api/apps/",
                              auth=settings["VALID_AUTH_TUPLE"])
        assert response.json() == [
            {'id': 'transport', 'display_name': 'transport', 'description': '', 'access_groups': []}]
        response = client.post("/api/apps/",
                               files={"app_data": open(
                                   validMiroAppPath, "rb")},
                               data={
        ...
        response = client.get("/api/apps/",
                              auth=settings["VALID_AUTH_TUPLE"])
        assert response.json() == [
            {'id': 'transport', 'display_name': 'transport',
                'description': '', 'access_groups': []},
            {'id': 'transport_test', 'display_name': 'My custom transport', 'description': 'This is my custom transport app', 'access_groups': ["mygroup"]}]
    ...
    def test_delete_app(self, cleanup):
        ...
        assert response.status_code == 201
        response = client.delete(
            "/api/apps/transport", params={"delete_data": True}, auth=settings["VALID_AUTH_TUPLE"])
        print(response.json())
        assert response.status_code == 200
    ...
    def test_update_app(self, cleanup):
        ...
        response = client.get("/api/apps/",
                              auth=settings["VALID_AUTH_TUPLE"])
        assert response.json() == [
            {'id': 'transport', 'display_name': 'My custom transport', 'description': 'This is my custom transport app', 'access_groups': ["mygroup"]}]
        scen_data = get_scen_metadata("transport")
        assert len(scen_data) == 1
        ...
        response = client.get("/api/apps/",
                              auth=settings["VALID_AUTH_TUPLE"])
        assert response.json() == [
            {'id': 'transport', 'display_name': 'Test123', 'description': 'This is my custom transport app', 'access_groups': []}]
