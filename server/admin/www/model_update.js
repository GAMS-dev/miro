...
/* harmony import */ var bootbox__WEBPACK_IMPORTED_MODULE_193__ = __webpack_require__(/*! bootbox */ "./node_modules/bootbox/bootbox.all.js");
/* harmony import */ var bootbox__WEBPACK_IMPORTED_MODULE_193___default = /*#__PURE__*/__webpack_require__.n(bootbox__WEBPACK_IMPORTED_MODULE_193__);
...
var $overlay = $('#overlayScreen');
var $loadingScreen = $('#loading-screen');
var $appsWrapper = $('#appsWrapper');
var appFilesPlaceholder = 'Drop your MIRO app here.';
var appNamePlaceholder = 'App title';
var appDescPlaceholder = 'Short model description (optional)';
var appGroupsPlaceholder = 'Access groups (optional)';
var appLogoPlaceholder = 'Different app logo? Drop your MIRO app logo here.';
var reorderAppsMode = false;
var currentConfigList = null;
var currentGroupList = [];
var currentAppId = null;
var currentAppLogo = null;
var addAppWrapperHTML = "<div id=\"addAppBox\" class=\"add-app-box app-box-fixed-height\">\n                             <div style=\"height:200px;\">\n                                 <p class=\"add-app-box-logo\">\n                                </p>\n                             </div>\n                             <div>\n                               <div class=\"add-app-box-title\"></div>\n                               <div class=\"add-app-box-desc\"></div>\n                             </div>\n                            <a class=\"btn-add-app\" id=\"addApp\"><i class=\"fas fa-plus-circle\"></i></a>\n                          </div>";

function sendAddRequest() {
  var removeInconsistentTables = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
  var newAppTitle = document.getElementById('newAppName').value;

  if (!newAppTitle || newAppTitle.trim() === '') {
    bootbox__WEBPACK_IMPORTED_MODULE_193___default.a.alert({
  ...
  Shiny.setInputValue('addApp', {
    ...
    removeInconsistentTables: removeInconsistentTables
  }, {
    priority: 'event'
  });
}
...
function sendUpdateRequest(index) {
  var newAppTitle = $("#appTitle_".concat(index)).val().trim();
  ...
  var newAppDesc = $("#appDesc_".concat(index)).val().trim();
  ...
  Shiny.setInputValue('updateApp', {
    ...
    groups: $("#appGroups_".concat(index)).val()
  }, {
    priority: 'event'
  });
}
...
function exitOverlayMode() {
  if ($('#expandedAddAppWrapper').is(':visible')) {
    ...
    Shiny.unbindAll(document.getElementById("appBox_".concat(currentAppId)));
  }
...
function refreshConfigList() {
  ...
  currentAppLogo = null;
  $appsWrapper.empty();
  var appsList = currentConfigList.reduce(function (html, configData, indexRaw) {
    var appNameSafe = escapeHtml(configData.alias);
    var descSave = escapeHtml(configData.desc);
    var id = configData.id;
    var index = indexRaw + 1;
    var groupOptions = configData.groups != null ? configData.groups.reduce(function (optionsHTML, groupName) {
      return "".concat(optionsHTML, "<option value=\"").concat(groupName, "\" selected>").concat(groupName, "</option>");
    }, '') : '';

    if (currentGroupList.length > 0) {
      var nonSelectedGroups = currentGroupList;

      if (configData.groups != null) {
        nonSelectedGroups = currentGroupList.filter(function (groupName) {
          return !configData.groups.includes(groupName);
        });
      }

      groupOptions += nonSelectedGroups.reduce(function (optionsHTML, groupName) {
        return "".concat(optionsHTML, "<option value=\"").concat(groupName, "\">").concat(groupName, "</option>");
      }, '');
    }

    return "".concat(html, "<div class=\"col-xxl-3 col-lg-4 col-sm-6 col-12 miro-app-item\" data-id=\"").concat(id, "\">\n        <div id=\"appBox_").concat(id, "\" class=\"app-box app-box-draggable launch-app-box app-box-fixed-height\" data-id=\"").concat(id, "\" data-index=\"").concat(index, "\" draggable=\"true\">\n          <div>\n            <div style=\"height:200px;\">\n              <div id=\"appLogo_").concat(id, "\" class=\"app-logo\" style=\"background-image:url(").concat(configData.logob64, ");\" data-id=\"").concat(id, "\">\n              </div>\n            </div>\n            <div>\n            <div style=\"height:200px\">\n              <h3 title=\"").concat(appNameSafe, "\" id=\"staticAppTitle_").concat(index, "\" class=\"app-title app-title-fixed app-item-title\" style=\"margin-top:15pt;width:100%;\">").concat(appNameSafe, "</h3>\n              <input id=\"appTitle_").concat(index, "\" value=\"").concat(appNameSafe, "\" required=\"required\" type=\"text\" name=\"alias\" class=\"app-title editable h3-style\" style=\"margin-top:15pt;width:100%;display:none;\" placeholder=\"").concat(appNamePlaceholder, "\">\n              <p title=\"").concat(descSave, "\" id=\"staticAppDesc_").concat(index, "\" class=\"app-desc app-desc-fixed app-item-desc\">").concat(descSave, "</p>\n              <textarea id=\"appDesc_").concat(index, "\" rows=\"3\" name=\"desc\" class=\"app-desc editable\" style=\"margin-bottom:1rem;width:100%;display:none;\" placeholder=\"").concat(appDescPlaceholder, "\">").concat(descSave, "</textarea>\n              <div id=\"appGroupsWrapper_").concat(index, "\" style=\"display:none;\">\n                <label for=\"newAppGroups\">").concat(appGroupsPlaceholder, "</label>\n                <select id=\"appGroups_").concat(index, "\" multiple>").concat(groupOptions, "</select>\n                <div style=\"height:1rem\"></div>\n              </div>\n            </div>\n          </div>\n        </div>\n        <div style=\"text-align:right;display:none;\" class=\"edit-bt-group\">\n            <input data-index=\"").concat(index, "\" data-id=\"").concat(id, "\" class=\"btn btn-secondary cancel-btn\" value=\"Cancel\" type=\"reset\">\n            <button class=\"btn btn-secondary confirm-btn btn-save-changes\" data-id=\"").concat(id, "\" data-index=\"").concat(index, "\" type=\"button\">Save</button>\n        </div>\n        <a class=\"delete-app-button app-corner-button\" data-index=\"").concat(index, "\"><i class=\"fas fa-times\"></i></a>\n      </div>\n    </div>");
  }, '');
  $appsWrapper.html("".concat(appsList, "<div id=\"addAppWrapper\" class=\"col-xxl-3 col-lg-4 col-sm-6 col-12\">").concat(addAppWrapperHTML, "</div>"));
...
$appsWrapper.on('dragenter', '#addAppBox', function () {
  expandAddAppForm();
});
...
$appsWrapper.on('change', '.input-app-logo', function () {
  $('.btn-save-changes').attr('disabled', true);
  $('#btAddApp').attr('disabled', true);
});
...
$appsWrapper.on('dragstart', '.app-box-draggable', function (e) {
...
$appsWrapper.on('dragover', '.app-box-draggable', function (e) {
  e.preventDefault();
  e.stopPropagation();
});
...
$appsWrapper.on('dragend', '.app-box-draggable', function (e) {
  if (!reorderAppsMode) {
    return;
  }
...
$appsWrapper.on('drop', '.app-box-draggable', function (e) {
  ...
  dragAddAppCounter = 0;
  $('.app-box-draggable').removeClass('drag-drop-area-dragover').css('opacity', '');
  var idFromRaw = e.originalEvent.dataTransfer.getData('text/plain');
  var idFrom = idFromRaw.slice(7);
  var idToRaw = $(this).attr('id');
  var idTo = idToRaw.slice(7);
...
$appsWrapper.on('click', '.app-box', function (e) {
  ...
  if ($overlay.is(':visible') || $target.hasClass('cancel-btn') || $target.hasClass('delete-app-button') || $target.parents('.delete-app-button').length) {
    return;
  }

  var $this = $(this);
  var appID = this.dataset.id;
  var appIndex = this.dataset.index;

  if (appIndex) {
    $("#appBox_".concat(appID)).removeClass('app-box-fixed-height');
    $("#appLogo_".concat(appID)).html("<label for=\"updateMiroAppLogo\" style=\"width:100%;height:100%;\">\n                  <div class=\"drag-drop-area-text not-empty\">\n                  ".concat(appLogoPlaceholder, "\n                  </div>\n                </label>\n                <input class=\"input-btn input-app-logo\" style=\"margin-top:1rem;\" type=\"file\" name=\"updateMiroAppLogo\" id=\"updateMiroAppLogo\" data-restore=\"\">")).addClass('drag-drop-area input-group').after("<div id=\"updateMiroAppLogo_progress\" class=\"progress active shiny-file-input-progress\">\n                  <div class=\"progress-bar\"></div>\n                </div>");
    $("#appTitle_".concat(appIndex)).show();
    $("#staticAppTitle_".concat(appIndex)).hide();
    $("#appDesc_".concat(appIndex)).show();
    $("#staticAppDesc_".concat(appIndex)).hide();
    $("#appGroupsWrapper_".concat(appIndex)).show();
    Shiny.bindAll(document.getElementById("appBox_".concat(appID)));
  }

  $("#appBox_".concat(appID, " .edit-bt-group")).slideDown(200);
  $this.css('z-index', 11);
  currentAppId = appID;
...
$appsWrapper.on('click', '.delete-app-button', function () {
  var appIndex = this.dataset.index;
  bootbox__WEBPACK_IMPORTED_MODULE_193___default.a.confirm({
    message: 'Are you sure you want to remove this app? This cannot be undone.',
    centerVertical: true,
    callback: function callback(removeAppConfirmed) {
      ...
      bootbox__WEBPACK_IMPORTED_MODULE_193___default.a.confirm({
        ...
        centerVertical: true,
        callback: function callback(removeDataConfirmed) {
...
$appsWrapper.on('change', '#newAppName', function () {
...
$appsWrapper.on('click', '#btAddApp', function () {
  sendAddRequest();
}); // Login form
...
$(document).ready(function () {
  ...
  Shiny.addCustomMessageHandler('onInit', function (data) {
  ...
  Shiny.addCustomMessageHandler('onLoginRequired', function (e) {
    // eslint-disable-line no-unused-vars
    $loadingScreen.fadeOut(200);
    openLoginForm();
  });
  Shiny.addCustomMessageHandler('onLoginSuccessful', function (e) {
    // eslint-disable-line no-unused-vars
    $('#loginModal').modal('hide');
  });
  Shiny.addCustomMessageHandler('onSuccess', function (data) {
    $loadingScreen.fadeOut(200);
    ...
    if (data.requestType === 'addApp') {
      ...
      if (Array.isArray(currentConfigList)) {
        var idxFrom = currentConfigList.findIndex(function (el) {
          return el.id === data.idFrom;
        });
        var idxTo = currentConfigList.findIndex(function (el) {
          return el.id === data.idTo;
        });
        var _ref = [currentConfigList[idxTo], currentConfigList[idxFrom]];
        currentConfigList[idxFrom] = _ref[0];
        currentConfigList[idxTo] = _ref[1];
      }

      $("#".concat(data.idFromRaw)).parent().swapWith($("#".concat(data.idToRaw)).parent());
      return;
    }
  ...
  Shiny.addCustomMessageHandler('onAddAppProgress', function (progress) {
    if (progress === -1) {
      $('#addAppProgress').css('width', '0%').attr('aria-valuenow', '0');
      $('#addAppSpinner').hide();
    } else {
      $('#addAppProgress').css('width', "".concat(progress, "%")).attr('aria-valuenow', progress);
    }
  });
  Shiny.addCustomMessageHandler('onInconsistentDbTables', function (data) {
    bootbox__WEBPACK_IMPORTED_MODULE_193___default.a.confirm({
      message: "Your database contains records that are inconsistent with the new version of the MIRO application you want to add. Do you want to remove all inconsistent data? The datasets to be removed are: '".concat(data.datasetsToRemove.join("', '"), "'."),
      centerVertical: true,
      callback: function callback(removeDataConfirmed) {
  ...
  Shiny.addCustomMessageHandler('onAddAppLogo', function (data) {
    if (!currentAppId) {
    ...
    if (currentAppId === -1) {
      // new App
      $('#btAddApp').attr('disabled', false);
      $('#newAppLogo').css('background-image', "url(\"".concat(data.logoB64, "\")"));
      return;
    }

    $('.btn-save-changes').attr('disabled', false);
    currentAppLogo = data.logoB64;
    $("#appLogo_".concat(currentAppId)).css('background-image', "url(\"".concat(currentAppLogo, "\")"));
  });
  Shiny.addCustomMessageHandler('onNewAppValidated', function (data) {
    var newAppTitle = document.getElementById('newAppName').value;
    ...
    if (data.appDesc) {
      var newAppDesc = document.getElementById('newAppDesc').value;
  ...
  Shiny.addCustomMessageHandler('onError', function (e) {
    $loadingScreen.fadeOut(200);
...
/***/ 0:
/*!******************************!*\
  !*** multi ./srcjs/index.js ***!
  \******************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! ./srcjs/index.js */"./srcjs/index.js");
...
/******/ });
//# sourceMappingURL=model_update.js.map