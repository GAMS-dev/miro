...
ipcRenderer.on('lang-data-received', (e, lang) => {
  ...
  https
    .get('https://gams.com/miro/latest.ver', (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          if (currentMajor === installedMajor) {
            ...
            if (
              currentMinor === installedMinor &&
              parseInt(currentVersion[2], 10) >
                parseInt(installedVersion[2], 10)
            ) {
              updateStatus(newVersionText, false);
              return;
            }
          ...
          __electronLog.info(
            `Problems getting latest MIRO version. Error message: ${err.message}.`,
          );
          updateStatus(lang.error);
        }
      });
