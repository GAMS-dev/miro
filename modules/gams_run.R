...
## asynchronous solve 
#observeEvent(input$btSolveAsync, {
#  if(async.confirmed){
#    removeModal()
#    errMsg <- NULL
#    # define directory to save input data to
#    asyncDir     <- paste0(asyncDir.name, modelName, "/")
#    asyncWorkDir <- paste0(asyncDir, uid, async.count, "/")
#    # count number of scenarios already committed
#    nbScenarios <- 0L
#    
#    #create workDir for asynchronous solves
#    if(!dir.create(asyncWorkDir, recursive = T, showWarnings = F)){
#      showModal(moNdalDialog(
#        title = lang$errMsg$fileWrite$title, HTML(addHtmlLineBreaks(sprintf(lang$errMsg$fileWrite$desc, asyncWorkDir, "")))
#      ))
#      return(NULL)
#    }
#    
...
#      # write csv files used to communicate with GAMS
#      tryCatch({
#        write.table(data.tmp[[i]], file = paste0(asyncWorkDir, names(data.tmp)[[i]], ".csv"), append = FALSE,
#                    row.names = FALSE, sep = config$csvDelim, na = "")
#      }, error = function(e) {
#        fileName <- paste0(tolower(names(modelOut)[[i]]), ".csv")
#        errMsg <<- paste(errMsg, sprintf(lang$errMsg$GAMSInput$writeCsv, fileName, conditionMessage(e)), sep = "\n")
#      })
#      
#    })
#    if(!is.null(errMsg)){
#      # remove tempDir
#      unlink(file.path(asyncWorkDir), recursive=TRUE)
#      
#      showModal(modalDialog(
#        title = lang$errMsg$GAMSInput$title, HTML(addHtmlLineBreaks(errMsg))
#      ))
#      return(NULL)
#    }
#    # write/append status file
#    if(!file.exists(paste0(asyncWorkDir, async.exec.file))){
#      tryCatch({
#        file.create(paste0(asyncWorkDir, async.exec.file))
#      }, warning = function(w){
#        errMsg <<- sprintf(lang$errMsg$fileWrite$desc, paste0(asyncWorkDir, async.exec.file), conditionMessage(w))
#      }, error = function(e){
#        errMsg <<- sprintf(lang$errMsg$fileWrite$desc, paste0(asyncWorkDir, async.exec.file), conditionMessage(e))
#      })
#    }
#    if(!is.null(errMsg)){
#      # remove tempDir
...
#      
#      showModal(modalDialog(
#        title = lang$errMsg$fileWrite$title, HTML(addHtmlLineBreaks(errMsg))
#      ))
#      return(NULL)
#    }
#    
#    tryCatch({
#      suppressWarnings(
#        nbScenarios <- readLines(paste0(asyncDir, async.exec.file))
#      )
#    }, error = function(e){
#      errMsg <<- sprintf(lang$errMsg$fileRead$desc, paste0(asyncDir, async.exec.file), conditionMessage(e))
#    })
#    if(!is.null(errMsg)){
#      # remove tempDir
#      unlink(file.path(asyncWorkDir), recursive=TRUE)
#      
#      showModal(modalDialog(
#        title = lang$errMsg$fileRead$title, HTML(addHtmlLineBreaks(errMsg))
#      ))
#      return(NULL)
#    }
#    
#    txt <- NULL
#    if(!length(nbScenarios)){
#      # file is empty so write gams execute command
#      txt <- paste0("gams ", modelName, ".gms ", "idir1=../../", modelDir, " --xlsname=", modelName, config$gamsWEBUISwitch)
#    }
#    
#    txt <- paste(txt, asyncWorkDir, sep = "\n")
#    tryCatch({
#      cat(txt, file = paste0(asyncDir, async.exec.file), append = T)
#    }, error = function(e){
#      errMsg <<- sprintf(lang$errMsg$fileWrite$desc, paste0(asyncDir, async.exec.file), conditionMessage(e))
#      # remove tempDir
#      unlink(file.path(asyncWorkDir), recursive=TRUE)
#      
#    })
...
#    }
#    
#    # successfully created input files so increment counter
#    async.count <<- async.count + 1
#    
#  }else{
#    shinyjs::show("divAsync")
#    shinyjs::hide("divDescGAMSrun")
#    shinyjs::hide("btSolveSync")
#    async.confirmed <<- T
#  }
#})