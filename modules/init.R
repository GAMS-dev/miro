    ...
    switch(modelIn[[i]]$type,
           dropdown = {
             ...
             if(names(modelIn)[[i]] %in% c(scalars.file.name, scalars.out.name)){
               errMsg <<- paste(errMsg, paste0("The dropdown menu: '", modelIn.alias[i], "' uses a reserved name as the identifier. Please choose a different name."), sep = "\n")
             }
             # get dependencies
             tryCatch({
             ...
             if(length(choices$shared > 0) && length(choices$shared) != 1){
               errMsg <<- paste(errMsg,paste0("The dropdown menu : '", modelIn.alias[i], "' refers to a shared database. However, currently a maximum of 1 column from an external source is supported."), sep = "\n")
               return(NULL)
             }else if(length(choices$shared) == 1){
               if(length(aliases$shared) > 0 && length(aliases$shared) != 1){
                 errMsg <<- paste(errMsg,paste0("The choices for dropdown menu '", modelIn.alias[i], "' do not match the number of aliases."), sep = "\n")
                 return(NULL)
             ...
             if(!is.null(aliases)){
               if(length(aliases$strings) != length(choices$strings)){
                 errMsg <<- paste(errMsg,paste0("The number of fixed aliases for dropdown menu: ", modelIn.alias[i], " does not match the number of choices without dependencies. 
                                                Aliases: '", paste(aliases$strings, collapse = ","), ". Choices: '", paste(choices$strings, collapse = ","), "'."), sep = "\n")
                 return(NULL)
               }else if(length(aliases$fw) != length(choices$fw)){
                 errMsg <<- paste(errMsg,paste0("The number of aliases with dependencies for dropdown menu: ", modelIn.alias[i], " does not match the number of choices with dependencies. 
                                                Aliases: '", paste(aliases$fw, collapse = ","), ". Choices: '", paste(choices$fw, collapse = ","), "'."), sep = "\n")
                 return(NULL)
                 # sheet names of aliases and choices do not match
               }else if(any(vapply(names(aliases$fw), function(sheet){if(sheet %in% names(choices$fw)) return(F) else return(T)}, logical(1)))){
             ...
             # track of those datasets that the dropdown depends on
             k <- match(names(choices$fw), names(modelIn))
             k <- k[!is.na(k)]
             for(l in seq_along(k)){
               dependent.datasets[[l]]       <<- c(dependent.datasets[[l]], i)      # TODO: find better solution
             }
           ...
           slider = {
             ...
             if(names(modelIn)[[i]] %in% c(scalars.file.name, scalars.out.name)){
               errMsg <<- paste(errMsg, paste0("The slider: '", modelIn.alias[i], "' uses a reserved name as its identifier. Please choose a different name."), sep = "\n")
             }
             tryCatch({
               slider.values[[name]] <<- get.dependencies.slider(min = modelIn[[i]]$slider$min, max = modelIn[[i]]$slider$max, def = modelIn[[i]]$slider$default, step = modelIn[[i]]$slider$step,
                                                                 modelIn = modelIn, list.of.operators = list.of.operators)
               # no dependencies for slider
               if(is.null(slider.values[[name]])){
                 
                 slider.values[[name]] <<- list("min" = as.numeric(modelIn[[i]]$slider$min), "max" = as.numeric(modelIn[[i]]$slider$max), 
                                                "def" = as.numeric(modelIn[[i]]$slider$default), "step" = as.numeric(modelIn[[i]]$slider$step))
                 if(suppressWarnings(any(is.na(slider.values[[name]])))){
                   errMsg <<- paste(errMsg, paste0("The slider: '", modelIn.alias[i], " has non numeric values. Please make sure min, max etc. are numeric."), sep = "\n")
                 }
               }else{
                 # track of those datasets that the slider depends on
                 k <- unique(vapply(slider.values[[name]], function(el){return(match(names(el)[1], names(modelIn))[1])}, integer(1L)))
                 k <- k[!is.na(k)]
                 for(l in seq_along(k)){
                   dependent.datasets[[l]]       <<- c(dependent.datasets[[l]], i)      # TODO: find better solution
                 }
           ...
           hot = {
             ...
             if(names(modelIn)[[i]] %in% c(scalars.file.name, scalars.out.name) && !identical(names(modelIn[[i]]$headers), scalars.file.headers)){
               warning(paste0(modelIn.alias[i], " is defined to be the scalar input dataset, but has incorrect headers. The headers were adjusted accordingly."))
           ...
           date = {
             if(names(modelIn)[[i]] %in% c(scalars.file.name, scalars.out.name)){
               errMsg <<- paste(errMsg, paste0("The date selector: '", modelIn.alias[i], "' uses a reserved name as its identifier. Please choose a different name."), sep = "\n")
             }
           ...
           daterange = {
             if(names(modelIn)[[i]] %in% c(scalars.file.name, scalars.out.name)){
               errMsg <<- paste(errMsg, paste0("The date range selector: '", modelIn.alias[i], "' uses a reserved name as its identifier. Please choose a different name."), sep = "\n")
             }
             # TODO : support dependency
             return(NULL)
           },
           checkbox = {
             if(names(modelIn)[[i]] %in% c(scalars.file.name, scalars.out.name)){
               errMsg <<- paste(errMsg, paste0("The checkbox: '", modelIn.alias[i], "' uses a reserved name as its identifier. Please choose a different name."), sep = "\n")
             }
             if(is.character(modelIn[[i]]$checkbox$value)){
               ...
               if(grepl("\\$+$", modelIn[[i]]$checkbox$value)){
                 errMsg <<- paste(errMsg,paste0("The checkbox: '", modelIn.alias[i], "' has a backward dependency assigned. Currently only forward dependencies are supported for checkboxes."), sep = "\n")
               }
               # END error checks
               ...
               if(!is.na(idx1)){
                 ...
                 errMsg <<- paste(errMsg,paste0("The dependent dataset for checkbox: '", modelIn.alias[i], "' could not be found. Please make sure you define a valid reference."), sep = "\n")
               }
             }
             return(NULL)
           }
      ...
      lapply(names(modelIn), function(sheet.dep){
        ...
        if(col %in% ddown.dep[[sheet.dep]]$bw[[sheet]]){
          if(length(cols.with.dep[[i]][[col]])){
            errMsg <<- paste(errMsg,paste0(col, " of input sheet ", sheet, "has more than one dependency. Only one backward dependency per column is allowed.", e), sep = "\n")
          }else{
...
if(is.null(errMsg)){
  ...
  scen.table.names    <- modelName %+% "_" %+% scen.table.names
...
if(is.null(errMsg)){
  save(modelIn, modelOut, config, lang, input.ds.names, modelOut.to.display, modelInTemplate, scenDataTemplate, isShinyProxy, modelIn.tabular.data, 
       shared.data, colSubset, modelIn.file.names, ddown.dep, aliases.no.dep, choices.no.dep, slider.values, config.graphs.out, config.graphs.in, 
       inputTabs, inputTabTitles, modelIn.with.dep, modelOut.alias, modelIn.must.import, modelIn.alias, DDPar, GMSOpt, currentModelDir, modelIn.to.import.alias,
       modelIn.to.import, scen.table.names, scen.table.names.to.display, serverOS, GAMSReturnCodeMap,
       file = rSaveFileName)
}