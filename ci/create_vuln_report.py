...
def create_vuln_df_container_scan(report_location, whitelist_location=None):
    container_name = report_location[len(CONTAINER_SCAN_REPORT_PREFIX) : -len(".json")]
    with open(report_location, "r") as f:
        vuln_raw = json.loads(f.read())
...
def main():
    ...
    reports_dfs = []
    reports_dfs.append(
        create_vuln_df_js_dep(
            JS_DEP_REPORT_LOCATION, "ci/vuln_whitelists/npm_audit_whitelist.json"
        )
    )
    reports_dfs.append(
        create_vuln_df_r_audit(
            R_AUDIT_REPORT_LOCATION, "ci/vuln_whitelists/r_audit_whitelist.json"
        )
    )

    container_scan_reports = [
        f for f in os.listdir(".") if f.startswith(CONTAINER_SCAN_REPORT_PREFIX)
    ]

    for container_scan_report in container_scan_reports:
        reports_dfs.append(
            create_vuln_df_container_scan(
    ...
    reports_dfs.append(
        create_vuln_df_pip_audit(
            PIP_AUDIT_REPORT_LOCATION, "ci/vuln_whitelists/pip_audit_whitelist.json"
        )
    )
    ...
    html_to_write = (
        html_to_write.replace('border="1"', "")
        .replace('class="dataframe"', 'class="table table-striped"')
        .replace(' style="text-align: right;"', "")
        .replace(
            "<th>Identified</th>",
            '<th style="white-space:nowrap;">Identified <a href="#" data-toggle="tooltip" data-placement="bottom" title="Number of vulnerabilities identified by the scanner"><i class="fas fa-info-circle"></i></a></th>',
        )
        .replace(
            "<th>Confirmed</th>",
            '<th style="white-space:nowrap;">Confirmed <a href="#" data-toggle="tooltip" data-placement="bottom" title="Number of vulnerabilities after manual review of vulnerabilities detected by the scanner"><i class="fas fa-info-circle"></i></a></th>',
        )
        .replace("True", '<i class="fas fa-check-circle"></i>')
    )
