...
hcubeImport <- HcubeImport$new(db, scalarsFileName, scalarsOutName,
  ...
  templates = setNames(
    c(modelInTemplate, modelOutTemplate, scalarsInTemplate),
...
observeEvent(virtualActionButton(rv$noInvalidData), {
  ...
  tryCatch(
    ...
    error = function(e) {
      flog.error(
        "Problems fetching duplicated Scenarios from database. Error message: %s.",
        conditionMessage(e)
      )
...
observeEvent(virtualActionButton(rv$btSave), {
  ...
  tryCatch(
    {
      hcubeImport$saveScenarios(hcubeTags,
        jobID = jobImportID, readPerm = uid,
        writePerm = uid, execPerm = uid, progressBar = prog
      )
    },
    error = function(e) {
      flog.error(
        "Problems importing scenarios. Error message: %s.",
        conditionMessage(e)
      )
...
observeEvent(input$btUploadHcube, {
  ...
  tryCatch(
    ...
    error = function(e) {
      ...
      flog.error(
        "Problems writing Hypercube job metadata to database. Error message: '%s'.",
        conditionMessage(e)
      )
      noErr <<- FALSE
    }
