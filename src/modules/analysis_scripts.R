...
if (length(config$scripts$base)) {
  observeEvent(input$runScript, {
    ...
    if (scriptOutput$isRunning(scriptIdChar)) {
      ...
      showElReplaceTxt(
        session, paste0("#scriptOutput_", scriptId, " .btn-run-script"),
    ...
    if (!dir.exists(scriptsPath)) {
      if (dir.exists(file.path(currentModelDir, paste0("scripts_", modelName)))) {
        ...
        flog.info(
          "No 'scripts_%s' directory was found. Did you forget to include it in the model assembly file ('%s_files.txt')?",
          modelName, modelName
        )
        hideEl(session, paste0("#scriptOutput_", scriptId, " .script-spinner"))
        showEl(session, paste0("#scriptOutput_", scriptId, " .out-no-data"))
    ...
    tryCatch(
      {
        ...
        gdxio$wgdx(file.path(workDir, paste0("scripts_", modelName), "data.gdx"),
          scenData$get("sb", includeHiddenScalars = TRUE),
          squeezeZeros = "n"
        )
      },
      error = function(e) {
        flog.error(
          "Problems writing gdx file for script: '%s'. Error message: '%s'.",
