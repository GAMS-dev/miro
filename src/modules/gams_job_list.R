  ...
  observeEvent(input$downloadJobData, {
    ...
    res <- tryCatch(worker$getJobResults(jID),
      error = function(e) {
        ...
        flog.error(
          "Problems initiating job results download of job ID: '%s'. Error message: '%s'.",
          jID, errMsg
        )
...
getJobProgress <- function(jID) {
  ...
  if (identical(length(jobProgress), 3L)) {
    noFail <- sprintf(
      lang$nav$hcubeMode$showJobProgressDialog$failedMsg,
