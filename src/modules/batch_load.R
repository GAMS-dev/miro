  ...
  if(length(input$batchCompareNameCols)){
    if(tryCatch({
      ...
      if(any(is.na(mapColIds))){
        stop(sprintf("Setting scenIdNameMap: Invalid column(s): %s. This is likely an attempt to tamper with the app!",
      ...
      if(any(is.na(rowIdsToPick))){
        stop(sprintf("Setting scenIdNameMap: Invalid sid(s): %s. This is likely an attempt to tamper with the app!",
