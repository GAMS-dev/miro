importHcJob <- function(filePath, jobMeta) {
  ...
  if (tryCatch(
    ...
    error_bad_format = function(e) {
      flog.warn(
        "Invalid Hypercube job archive file format: %s",
        filePath
      )
      showErrorMsg(
        lang$errMsg$hcubeImport$invalidJob$title,
    ...
    error_invalid_results = function(e) {
      flog.warn(
        "Invalid Hypercube job archive. Error message: %s",
        conditionMessage(e)
      )
      showErrorMsg(
        lang$errMsg$hcubeImport$invalidJob$title,
    ...
    error = function(e) {
      flog.error(
        "Unexpected error while validating Hypercube job archive. Error message: %s",
        conditionMessage(e)
      )
      showErrorMsg(
        lang$errMsg$hcubeImport$extract$title,
  ...
  tryCatch(
    {
      hcubeResults$extractArchive(session, workDir,
        progressCallback = function(progress) {
          prog$set(
            message = lang$progressBar$hcubeImport$title,
        ...
        callbackError = function(procStat, errMsg) {
          flog.error(
            "Problems extracting Hypercube job results archive. Return code: %s, stderr: %s",
            procStat, errMsg
          )
          showErrorMsg(
            lang$errMsg$hcubeImport$extract$title,
        ...
        callbackSuccess = function() {
          if (tryCatch(
            ...
            error_db = function(e) {
              showErrorMsg(
                lang$errMsg$hcubeImport$dbUpload$title,
            ...
            error = function(e) {
              flog.error(
                "Unexpected error while writing Hypercube job to database. Error message: %s",
                conditionMessage(e)
              )
              showErrorMsg(
                lang$errMsg$hcubeImport$dbUpload$title,
    ...
    error_extraction = function(e) {
      flog.warn(
        "Couldn't extract Hypercube job archive. Error message: %s",
        conditionMessage(e)
      )
      showErrorMsg(
        lang$errMsg$hcubeImport$extract$title,
