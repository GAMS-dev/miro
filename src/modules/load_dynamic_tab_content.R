...
loadDynamicTabContent <- function(session, tabsetId, sheetNames, initEnv = FALSE) {
  ...
  if (is.null(dynamicUILoaded$dynamicTabsets[[tabsetIdChar]])) {
    dynamicUILoaded$dynamicTabsets[[tabsetIdChar]] <<- list(
      ui = vector("logical", length(configGraphsOut) + length(modelIn)),
  ...
  for (sheetName in sheetNames) {
    ...
    if (isInPivotComp) {
      ...
      if (identical(sheetName, scalarsFileName)) {
        ...
        if (is.na(sheetId)) {
          ...
          if (is.na(sheetId)) {
            flog.error(
              "Sheet name: '%s' could not be found in either names of input or output symbols. Should never happen!",
              sheetName
            )
            return(showErrorMsg(
              lang$errMsg$loadScen$title,
    ...
    if (dynamicUILoaded$dynamicTabsets[[tabsetIdChar]][["ui"]][tabId]) {
      ...
      flog.trace(
        "Rendering UI elements for tabset: %s on tab: %s (sheetname: %s)",
      ...
      tryCatch(
        ...
        error = function(e) {
          flog.error(
            "Problems rendering UI elements for scenario dataset: '%s'. Error message: %s.",
      ...
      if (!isInPivotComp) {
        tryCatch(
          ...
          error = function(e) {
            flog.error(
              "Problems rendering table UI for scenario dataset: '%s'. Error message: %s.",
              sheetName, conditionMessage(e)
            )
            errMsg <<- lang$errMsg$loadScen$desc
          }
    ...
    if (!dynamicUILoaded$dynamicTabsets[[tabsetIdChar]][["content"]][tabId]) {
      ...
      if (isInPivotComp) {
        tryCatch(
          ...
          error = function(e) {
            flog.error(
              "Problem rendering graphs for dataset: '%s'. Error message: %s.",
              sheetName, conditionMessage(e)
            )
