removeDbTablesButton <- function(id){
  ns <- NS(id)
  actionButton(ns("btRemoveAllDbTables"), class = "bt-remove",
               ...
               function(input, output, session){
                 showRemoveAllDbTablesDialog <- function(){
                   showModal(modalDialog(
                     ...
                     checkboxInput_MIRO(session$ns("cbRemoveAllDbTables"),
                   ...
                   tryCatch({
                     ...
                     if(!is.null(successContainerId)){
                       if(is.null(returnCode)){
                         showHideEl(session, paste0("#", successContainerId),
                                    4000L, lang$nav$migrationModule$btRemoveTables$msgSuccess)
                       }else{
                         showElReplaceTxt(session, paste0("#", successContainerId),
                     ...
                     flog.error("Problems removing database tables. Error message: %s",
                                conditionMessage(e))
                     if(!is.null(errorContainerId)){
                       showHideEl(session, paste0("#", errorContainerId),
