dbMigrationForm <- function(id, inconsistentTablesInfo, orphanedTablesInfo,
  ...
  tagList(
    ...
    tags$div(
      id = ns("migFormContainer"), class = "container-fluid", style = "font-size:12pt;",
      tags$div(
        id = ns("migrationSuccess"), class = "gmsalert gmsalert-success",
        style = "position:fixed;",
        lang$nav$migrationModule$successMsg
      ),
      tags$div(
        id = ns("dataMigrationErrors"), class = "gmsalert gmsalert-error",
        ...
        tagList(
          tags$div(
            ...
            tags$h3(
              ...
              sprintf(
                lang$nav$migrationModule$labelSymbol,
                tableInfo$tabName
              )
            )
          ),
      ...
      tags$div(
        class = "row", style = "padding: 15px;text-align: right",
        actionButton(ns("btConfirmMigration"),
          class = "bt-highlight-1",
    ...
    function(input, output, session) {
      ...
      getMigrationConfig <- reactive({
        ...
        isolate({
          ...
          migrationConfig <- lapply(seq_along(inconsistentTablesInfo), function(i) {
            tableInfo <- inconsistentTablesInfo[[i]]
            tableConfig <- list(
              oldTableName = input[[paste0("dbMigrateTable_", i)]],
            ...
            if (length(missingCols)) {
              errMsgDataLoss <<- paste(errMsgDataLoss,
                sprintf(
                  lang$nav$migrationModule$dialogConfirmDataLoss$descTableCols,
          ...
          if (showDataLossErrors && any(tablesToBeRemoved)) {
            errMsgDataLoss <- paste(errMsgDataLoss,
              sprintf(
                lang$nav$migrationModule$dialogConfirmDataLoss$descTables,
          ...
          if (any(duplicated(tablesMigrated))) {
            stop_custom("error_bad_settings",
              sprintf(
                lang$nav$migrationModule$errDuplicateTables,
      ...
      if (standalone) {
        ...
        observeEvent(input$btConfirmDataLoss, {
          ...
          updateProgress <- function() {
            progress$inc(1 / length(inconsistentTablesInfo),
              detail = sprintf(
                lang$nav$migrationModule$progress$desc,
          ...
          tryCatch(
            {
              ...
              showElReplaceTxt(
                session, paste0("#", session$ns("migrationSuccess")),
                lang$nav$migrationModule$successMsg
              )
              returnCode(0L)
            },
            error_bad_settings = function(e) {
              ...
              showHideEl(
                session, paste0("#", session$ns("dataMigrationErrors")),
        ...
        removeDbTablesServer("removeAllButton",
          errorContainerId = session$ns("dataMigrationErrors"),
