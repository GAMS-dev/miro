mirorenderer_repOutput <- function(id, height = NULL, options = NULL, path = NULL){
  ...
  tagList(
    ...
    tags$div(class = "container custom-css", style = "width:100%;padding-left: 0;padding-right: 0;",
             fluidRow(id = ns("valueboxes"), class="display-flex valueboxes custom-flex-row", style="margin:0px;",
                      column(12, class = "box-styles col-lg-3 col-md-6 col-sm-6",
             ...
             fluidRow(class = "custom-flex-row", style="margin:0px;",
                      column(4, class = "col-lg-4 col-md-6 col-sm-12 col-xs-12 col-lg-push-8", style="flex-grow: 1;",
                             tags$h4("Settings", class="highlight-block"),
                             tags$div(class = "row", style="margin:0px;",
                              tags$div(class = "custom-dropdown custom-dropdown-full-width", style = "float:right;",
                                     selectInput(ns("settingsType"), label = NULL,
                                                 choices = c("Settings", "Yield Factor realization"), selected = "Settings"))),
                             tags$div(style = "overflow: hidden;max-height:220px;",
                                      tableOutput(ns("settings")),
                                      plotly::plotlyOutput(ns("yfRealizations"), height=220))
                      ),
                      column(4, class = "col-lg-4 col-md-6 col-sm-12 col-xs-12 col-lg-pull-4", style="flex-grow: 1;",
             ...
             fluidRow(class = "custom-flex-row", style="margin:0px;",
                      column(12, style="flex-grow: 1; min-width:400px; margin-top:15px;",
                             tags$h4("Crop Report", class="highlight-block"),
                             tabsetPanel(type = "tabs",
                                tabPanel("Yield / Planted / Seed Cost",
                                    tags$div(class = "custom-dropdown", style = "float:right;",
                                             selectInput(ns("chartTypeYield"), label = NULL,
                                                         choices = c("Pie", "Bar"), selected = "Bar")),
                                    fluidRow(class = "custom-flex-row",
                                ...
                                tabPanel("Sales / Purchases",
                                         tags$div(class = "custom-dropdown", style = "float:right;",
                                                  selectInput(ns("chartTypeSales"), label = NULL,
                                                              choices = c("Pie", "Bar"), selected = "Bar")),
                                         fluidRow(class = "custom-flex-row",
...
renderMirorenderer_rep <- function(input, output, session, data, options = NULL, path = NULL, rendererEnv = NULL, views = NULL, outputScalarsFull = NULL, ...){
  ...
  settingsData <- outputScalarsFull %>% filter(scalar != "_gmsopt_emp") %>% select(c("description", "value"))
  kpiData <- data$rep
  cdData <- data$cd
  ...
  output$settings <- renderTable(settingsData, spacing = "xs",
  ...
  output$yfRealizations <- plotly::renderPlotly({
    ...
    dataTmp <- data$s_yf
    plotlyObj <- plot_ly(dataTmp, type = 'histogram', histnorm = "probability",
  ...
  infoBoxCustom <- function(value = NULL, symbol = NULL, position = "end",
    ...
    boxContent <- div(
      ...
      span(
        class = "info-box-icon",
        class = if (!fill) colorClass,
        style = if(!is.null(customColor)) paste0("background-color:", customColor, "!important;")
        else "",
        icon
      ),
      div(class = "info-box-content",
          span(class = "info-box-text", title),
          if (!is.null(value)) span(class = "info-box-number",
                                    if(!is.null(symbol)){
  ...
  lapply(1:length(valBoxNames), function(i) {
    output[[valBoxNames[i]]] <- renderValueBox({
      valueTmp <- kpiData %>% dplyr::filter(rh == valBoxNames[i]) %>% pull(value)
      infoBoxCustom(value = format(round(valueTmp, 0), big.mark = "."),
  ...
  output$kpiTitle <- renderText({
    ...
    titleTmp <- valBoxTitles[match(reportToRender,
                                   valBoxNames)]
  ...
  output$kpiCharts <- plotly::renderPlotly({
    ...
    plotlyObj <- plot_ly(dataTmp, type = 'histogram', histnorm = "probability",
      ...
      layout(yaxis = list(title = "Probability",
             ...
             xaxis = list(title = paste(toupper(substr(reportToRender, 1, 1)), substr(reportToRender, 2, nchar(reportToRender)), " [€]", sep=""),
                          zeroline = FALSE,
                          titlefont = list(
                            family = "'Helvetica Neue', 'Helvetica', 'Arial', sans-serif",
                            size = 14
