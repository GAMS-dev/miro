...
from gamspy import Container, Equation, Model, Parameter, Sense, Set, Sum, Variable, Smax
...
def main():
    ...
    # Set
    i = Set(m, name="i", records=["seattle",
            "san-diego"], description="canning plants")
    j = Set(m, name="j", records=["new-york",
    ...
    a = Parameter(m, name="a", domain=[
                  i], records=capacities, domain_forwarding=True, is_miro_input=True, description="capacity of plant i in cases")
    b = Parameter(m, name="b", domain=[
                  j], records=demands, domain_forwarding=True, is_miro_input=True, description="demand at market j in cases")
    d = Parameter(m, name="d", domain=[i, j],
    ...
    x = Variable(m, name="x", domain=[
                 i, j], type="Positive", description="shipment quantities in cases")
    z = Variable(
        m, name="z", description="total transportation costs in thousands of dollars")

    # Equation
    supply = Equation(m, name="supply", domain=[
                      i], description="observe supply limit at plant i")
    demand = Equation(m, name="demand", domain=[
                      j], description="satisfy demand at market j")
    ...
    ship = Variable(m, name="ship", domain=[
                    i, j], type="binary", description="1 if we ship from i to j, otherwise 0")

    minship = Equation(m, name="minship", domain=[
                       i, j], description="minimum shipment")
    maxship = Equation(m, name="maxship", domain=[
                       i, j], description="maimum shipment")
    ...
    costnlp = Equation(m, name="costnlp",
                       description="define non-linear objective function")
