XlsIO <- R6::R6Class("XlsIO",
  inherit = LocalFileIO, public = list(
    ...
    write = function(path, dataToWrite, metaData = NULL, includeMetadataSheet = FALSE, includeEmptySheets = TRUE) {
      ...
      if (any(isOutputSheet)) {
        wsNamesTmp[isOutputSheet] <- paste0(
          wsNamesTmp[isOutputSheet],
      ...
      dataToWrite <- c(
        dataToWrite,
        setNames(
          list(private$genIndexFromMetadata(
            wsNamesRaw[!emptySheets],
      ...
      if (isTRUE(includeMetadataSheet) && !is.null(metaData)) {
        ...
        symDesc <- bind_rows(lapply(seq_along(wsNamesRaw), function(idx) {
          tibble(
            description = private$metadata[[wsNamesRaw[idx]]]$alias,
        ...
        names(symDesc)[1:2] <- c(
          lang$nav$excelExport$metadataSheet$symbol,
        ...
        dataToWrite <- c(setNames(
          list(metaData),
          paste0(" ", lang$nav$excelExport$metadataSheet$title)
        ), dataToWrite)
      }
  ...
  private = list(
    ...
    getRangeSymbol = function(symName) {
        ...
        rowsToIgnore <- tryCatch(private$rangeToIndex(index$ignorerows, allowLetters = FALSE),
          error_parse_config = function(e) {
            stop_custom("error_parse_config",
              sprintf(lang$errMsg$xlsio$errors$badSymbolRange, symName),
              call. = FALSE
            )
          }
        )
      ...
      return(list(
        range = range, rowsToIgnore = rowsToIgnore,
    ...
    readFromIndex = function(symName) {
        ...
        if (index$cdim > 1L) {
          if (private$isTable(symName)) {
            data <- tidyr::pivot_longer(data, -seq_len(index$dim - index$cdim),
              names_to = c(
                names(private$metadata[[symName]]$headers)[seq(index$dim - index$cdim + 1L, index$dim - 1L)],
                ".value"
              ),
      ...
      if (private$isTable(symName)) {
        ...
        if (any(!tolower(dataHdr) %in% tolower(pivotedHeaders))) {
          stop_custom("error_data",
            sprintf(
              lang$errMsg$xlsio$errors$invalidRecords, symName,
              paste(dataHdr[!tolower(dataHdr) %in% tolower(pivotedHeaders)], collapse = "', '")
            ),
            call. = FALSE
          )
        }
      ...
      if (private$isTable(symName)) {
        numericalCols <- seq(
          stri_count_fixed(private$metadata[[symName]]$colTypes, "c") + 1L,
          length(data)
        )
    ...
    readScalars = function(symName) {
        ...
        if (any(duplicatedSym)) {
          stop_custom("error_data",
            sprintf(
              ...
              paste(scalarsDfTmp[[1]][duplicatedSym], collapse = "', '")
            ),
            call. = FALSE
          )
        }
      ...
      if (length(additionalScalarsToProcess)) {
        scalarsDf[match(
          additionalScalarsToProcess,
          scalarsToProcess
        ), 3] <- vapply(additionalScalarsToProcess,
    ...
    readScalarsNoIndex = function(symName) {
      if (!symName %in% names(private$metadata)) {
        stop_custom("error_notfound", sprintf(
          lang$errMsg$xlsio$errors$symbolNotFound,
          symName
        ), call. = FALSE)
      }
      scalarsToProcess <- private$metadata[[symName]]$symnames
      ...
      if (length(scalarXlsSheetIds)) {
        scalarsDf[match(
          tolower(private$rSheetsNoIndex)[scalarXlsSheetIds],
          scalarsToProcess
        ), 3] <- vapply(scalarXlsSheetIds, function(sheetID) {
    ...
    readVEScalars = function(symName) {
      ...
      if (symName %in% names(private$rIndex)) {
        ...
        if (any(duplicatedSym)) {
          stop_custom("error_data",
            sprintf(
              lang$errMsg$xlsio$errors$duplicateScalarVE,
              paste(scalarsDf[[1]][duplicatedSym], collapse = "', '")
            ),
            call. = FALSE
          )
        }
    ...
    getColHeaders = function(symName, rangeInfo) {
      index <- private$rIndex[[symName]]
      headerData <- private$readInternal(private$rpath,
        range = rangeInfo$headerRange,
    ...
    parseIndex = function(indexDf) {
      ...
      if (any(invalidRowIds)) {
        ...
        if (length(invalidSymbols)) {
          private$warnings$push(sprintf(
            lang$errMsg$xlsio$warnings$invalidSymbols,
      ...
      if (any(duplicateSymbols)) {
        stop_custom("error_parse_config", sprintf(
          lang$errMsg$xlsio$errors$duplicateSymbols,
          paste(symbolsInExcel[duplicateSymbols], collapse = "', '")
        ), call. = FALSE)
      }
      ...
      if ("se" %in% names(indexDf)) {
        ...
        if (any(invalidValueOptions)) {
          private$warnings$push(sprintf(
            lang$errMsg$xlsio$warnings$invalidSkipEmpty,
      ...
      if ("squeeze" %in% names(indexDf)) {
        ...
        if (any(invalidValueOptions)) {
          private$warnings$push(sprintf(
            lang$errMsg$xlsio$warnings$invalidSqueeze,
            paste(symbolsInExcel[invalidValueOptions], collapse = "', '")
          ))
        }
      ...
      if ("cdim" %in% names(indexDf)) {
        if ("rdim" %in% names(indexDf)) {
          ...
          if (any(invalidRdim)) {
            stop_custom("error_parse_config", sprintf(
              lang$errMsg$xlsio$errors$indexInvalidRdim,
              paste(symbolsInExcel[invalidRdim], collapse = "', '")
            ), call. = FALSE)
          }
          ...
          if (any(invalidCdim)) {
            ...
            if (length(invalidCdimSym)) {
              stop_custom("error_parse_config", sprintf(
                lang$errMsg$xlsio$errors$indexInvalidCdim,
                paste(invalidCdimSym, collapse = "', '")
              ), call. = FALSE)
            }
          }
          ...
          if (any(invalidDim)) {
            stop_custom("error_parse_config", sprintf(
              lang$errMsg$xlsio$errors$indexInvalidDim,
              paste(symbolsInExcel[!missingDim][invalidDim], collapse = "', '")
            ), call. = FALSE)
          }
        } else {
          missingcDim <- is.na(indexDf[["cdim"]])
          ...
          if (any(invalidCdim)) {
            stop_custom("error_parse_config", sprintf(
              lang$errMsg$xlsio$errors$indexInvalidCdim,
              paste(symbolsInExcel[invalidCdim], collapse = "', '")
            ), call. = FALSE)
          }
        }
      ...
      if ("values" %in% names(indexDf)) {
        ...
        if (any(invalidValueOptions)) {
          stop_custom("error_parse_config", sprintf(
            lang$errMsg$xlsio$errors$indexInvalidValues,
            paste(symbolsInExcel[invalidValueOptions], collapse = "', '")
          ), call. = FALSE)
        }
      } else {
    ...
    genIndexFromMetadata = function(symbolNames, wsNames, data) {
      return(bind_rows(lapply(seq_along(symbolNames), function(idx) {
        ...
        if (tolower(symName) %in% c(scalarsFileName, scalarsOutName)) {
          ...
          return(tibble(
            type = vapply(scalarTypes[scalarIds], function(symType) {
        ...
        return(tibble(
          type = if (symMeta$symtype %in% c("parameter", "equation", "variable")) "par" else "set",
          symbol = symName,
