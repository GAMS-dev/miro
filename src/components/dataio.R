  ...
  import = function(item, dsName) {
    ...
    if (identical(item$source, "customFunction")) {
      ...
      tryCatch(
        ...
        error = function(e) {
          stop(sprintf(
            "Problems importing data via custom import function: '%s'. Error message: %s",
            item$functionName, conditionMessage(e)
          ), call. = FALSE)
        }
      )
    ...
    switch(tolower(item$source),
      customfunction = {
        ...
        tryCatch(
          ...
          error = function(e) {
            stop(sprintf(
              "Problems exporting data via custom export function: '%s'. Error message: %s",
              item$functionName, conditionMessage(e)
            ), call. = FALSE)
          }
        )
