...
Db <- R6Class("Db",
              public = list(
                  ...
                  metadataRow <- self$importDataset("_scenMeta",
                                                    tibble(
                                                      c("_uid",
                                                        ...
                                                        "_scode"),
                                                      c(uid, sname,
                                                        if(private$hcubeActive) SCODEMAP[['hcube_jobconfig']]
                                                        else SCODEMAP[['scen']])),
                                                    limit = 1L)
                ...
                deleteRows        = function(tableName, colNames = NULL, values = NULL,
                  ...
                  if(!is.null(colNames)){
                    ...
                    stop(sprintf("Can't delete entire table: '%s'. Please specify subset.",
                                 tableName), call. = FALSE)
                  }
                  #END error checks
                  ...
                  if(!is.null(subsetSids) && length(subsetSids) >= 1L){
                    subsetSidSQL <- paste0(DBI::dbQuoteIdentifier(private$conn, "_sid"),
                                           " IN (",
                  ...
                  if(DBI::dbExistsTable(private$conn, tableNameDb)){
                    tryCatch({
                      query <- paste0("DELETE FROM ", DBI::dbQuoteIdentifier(private$conn, tableNameDb),
                                      subsetCondition)
                ...
                updateRows = function(tableName, ..., colNames, values, innerSepAND = TRUE, subsetSids = NULL){
                  ...
                  tryCatch({
                    ...
                    flog.debug("Db: %s rows in table: '%s' were updated (Db.updateRows)",
                               affectedRows, tableNameDb)
                  }, error = function(e){
                    stop(sprintf("Db: An error occurred while querying the database (Db.updateRows, table: '%s'). Error message: %s.",
          ...
          writeMetadata = function(metadata, update = FALSE){
            ...
            if(update){
              ...
              query   <- DBI::sqlInterpolate(private$conn, sql, uid = metadata[["_uid"]],
        ...
        metaTabName, conditionMessage(e)), call. = FALSE)
              })
            }else{
              # write new metadata
              tryCatch({
