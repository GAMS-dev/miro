  ...
  public = list(
    ...
    fetchResults = function(subsetList, colNames, limit = 1e5) {
      ...
      tryCatch(
        ...
        error = function(e) {
          stop(sprintf(
            "Db: An error occurred while querying the database (BatchLoader.fetchResults). Error message: '%s'.",
            conditionMessage(e)
          ), call. = FALSE)
        }
      )
    ...
    genGdxFiles = function(scenIds, tmpDir, gdxio, progressBar = NULL,
                           genScenList = FALSE) {
      stopifnot(
        length(scenIds) >= 1L, is.character(tmpDir),
        length(tmpDir) == 1L
      )
      ...
      if (any(sidsToFetch)) {
        sidsToFetch <- c(
          setNames(
            as.character(scenIdNameMap[[3]][sidsToFetch] - 10000L),
            scenIdsOrdered[sidsToFetch]
          ),
          setNames(
            as.character(scenIdsOrdered[!sidsToFetch]),
            scenIdsOrdered[!sidsToFetch]
          )
        )
      } else {
      ...
      dataTmp <- lapply(scenTableNames, function(tableName) {
        if (!is.null(progressBar)) {
          progressBar$inc(
            ...
            message = sprintf(
              "Importing table %s of %d from database.",
              j, noScenTables
            )
          )
          j <<- j + 1L
      ...
      for (tabId in seq_along(scenTableNames)) {
        ...
        if (!is.null(progressBar)) {
          progressBar$inc(
            amount = 1 / noProgressSteps,
            message = sprintf(
              "Importing table %d of %d from database.",
              tabId, noScenTables
            )
          )
        }
        if (scenTableNames[[tabId]] %in% c(
          scalarsFileName,
          names(ioConfig$modelOut)
        )) {
          subsetSids <- scenIds
        ...
        if (identical(tableName, "_metadata_")) {
          ...
          if (any(staticInputSids)) {
            staticInputSids <- setNames(
              as.character(tableTmp[["_scode"]][staticInputSids] - 10000L),
        ...
        lapply(seq_along(tableTmp), function(i) {
          ...
          if (identical(tabId, 1L)) {
            ...
            if (!dir.create(dirNameScen)) {
              stop(sprintf(
                "Temporary folder: '%s' could not be created.",
                dirNameScen
              ), call. = FALSE)
            }
  ...
  private = list(
    ...
    buildQuery = function(subsetList, colNames, limit) {
      ...
      if (length(private$scalarTables)) {
        escapedScalarsTableNames <- dbQuoteIdentifier(
          private$conn,
      ...
      query <- SQL(paste0(
        "SELECT ", escapedColNamesToFetch, " FROM ",
