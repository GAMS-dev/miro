HcubeLoad <- R6Class("HcubeLoad", 
                     public = list(
                       ...
                       fetchResults = function(subsetList, colNames, limit = 1e5){
                         ...
                         tryCatch({
                           flog.trace("Running query: '%s' (HcubeLoad.fetchResults).", query)
                           data <- as_tibble(DBI::dbGetQuery(private$conn, query))[, colNames]
                           flog.debug("Db: Data was imported (HcubeLoad.fetchResults).")
                         }, error = function(e){
                           stop(sprintf("Db: An error occurred while querying the database (HcubeLoad.fetchResults). Error message: '%s'.",
