  ...
  if (length(metadata[["attachments"]])) {
    ...
    if (length(attachmentMetadata[[1]])) {
      if (length(zipPath)) {
        ...
        unzip(zipPath,
          files = paste0("attachments/", attachmentMetadata[[1]]),
...
loadMiroScen <- function(path, activeScen, attachments, views, inputNames, exdir = NULL, loadMetadata = TRUE) {
  ...
  on.exit(unlink(tmpd, recursive = TRUE, force = TRUE))
  unzip(path,
    files = c("metadata.json", "views.json", "data.gdx"),
  ...
  if (tmpd != exdir && !file.move(
    file.path(tmpd, "data.gdx"),
    file.path(exdir, "data.gdx")
  )) {
    stop_custom("error_os", sprintf(
...
generateMiroScenMeta <- function(path, metadata, attachments, views,
  ...
  if (length(attachmentMetadata) && length(attachmentMetadata[[1]])) {
    if (length(attachments$download(file.path(path, "attachments"),
      ...
      scenId = attachmentSids
    )) != length(attachmentMetadata[[1]])) {
      stop("Unexpected error occurred while downloading attachments.", call. = FALSE)
    }
  }
  metadataContent <- list(
    version = 1L, scen_name = if (is.null(jobName)) metadata[[3]][1] else jobName,
