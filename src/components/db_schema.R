DbSchema <- R6Class("DbSchema", public = list(
  ...
  getCreateTableQueryRaw = function(symName, includeForeignKey = TRUE,
    ...
    return(paste0(
      ...
      " (", paste(dbQuoteIdentifier(private$conn, symSchema$colNames),
        self$getColTypesSQL(symSchema$colTypes),
        collapse = ", "
      ),
      foreignKeyConstraint, ");"
    ))
  ...
  getColTypesSQL = function(colTypes) {
    return(vapply(strsplit(colTypes, "", fixed = TRUE)[[1]], function(colType) {
      ...
      stop_custom("error_config", sprintf(
        "Invalid migration config: Invalid column type: %s",
        colType
      ),
      call. = FALSE
      )
  ...
  getCreateScalarViewQuery = function(viewName, scalarNames) {
    ...
    return(paste0(
      "CREATE VIEW ", dbQuoteIdentifier(private$conn, viewName), " AS SELECT ",
  ...
  getCreateScalarViewTriggerQuery = function(viewName, scalarNames) {
    ...
    return(paste0(
      "CREATE TRIGGER ", dbQuoteIdentifier(private$conn, paste0(viewName, "_insert")),
  ...
  getCreateHcScalarsTableQuery = function() {
    return(self$getCreateTableQueryRaw("_hc__scalars",
      dbTableName = "_hc__scalars",
      symSchema = list(
        tabName = "_hc__scalars",
        colNames = c(
          "_sid", "scalar",
          "description", "value"
        ),
