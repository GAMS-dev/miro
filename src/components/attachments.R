Attachments <- R6Class("Attachments",
                       ...
                       public = list(
                         ...
                         flushOpQueue = function(){
                           ...
                           opQueue <- list(remove = private$attachmentsToRemove,
                         ...
                         save = function(filePaths, fileNames, overwrite = TRUE, fullPath = NULL){
                           ...
                           if(length(filePaths) == 1L){
                             ...
                             stop_custom("error_file_exists",
                                         "File(s) already exist(s)", call. = FALSE)
                           }
                         ...
                         add = function(session, filePaths, fileNames = NULL, overwrite = FALSE, execPerm = NULL){
                           ...
                           if(!is.null(session) && self$isReadonly(session)){
                             stop_custom("error_readonly",
                                         "Tried to modify attachment data from readonly tab.", call. = FALSE)
                           }
                           if(length(filePaths) < 1L){
                             return(invisible(self))
                           }
                           ...
                           if(length(private$config$forbiddenFNames) && 
                             ...
                             stop_custom("error_forbidden_filename",
                                         "Some of the filenames specified are not allowed to be used", call. = FALSE)
                           }
                           fileSize <- file.size(filePaths)
                           
                           if(any(is.na(fileSize))){
                             stop_custom("error_file_not_found",
                                         "Some of the files to add do not exist", call. = FALSE)
                           }
                           
                           if(any(fileSize > private$config[["maxSize"]])){
                             stop_custom("error_max_size",
                                         "Some of the files exceed the maximum allowed attachment size", call. = FALSE)
                           }
                           ...
                           if(any(filesNeedRelocation)){
                             ...
                             if(any(failedFileRelocations)){
                               flog.warn("Some attachments could not be relocated: '%s' (Attachments.add).", 
                         ...
                         setExecPerm = function(session, fileNames, execPerm){
                           ...
                           stopifnot(is.character(fileNames), is.logical(execPerm),
                           ...
                           if(any(is.na(localFileIds))){
                             ...
                             if(any(is.na(remoteFileIds))){
                               stop_custom("error_file_not_found",
                                           sprintf("Some of the files (%s) do not exist",
                                                   paste(fileNames[is.na(remoteFileIds)])), call. = FALSE)
                             }
                           ...
                           failedFileMoves <- !file.move(private$localAttachments$filePaths[localFileIds], 
                                                         newPaths)
                           if(any(failedFileMoves)){
                             flog.warn("Problems moving attachment(s): '%s' (Attachments.setExecPerm).",
                       ...
                       private = list(
                         ...
                         removeLocalFiles = function(){
                           if(length(private$localAttachments$filePaths)){
                             ...
                             if(any(!removedLocalFiles))
                               flog.warn("Some local attachments could not be removed: '%s' (Attachments.removeLocalFiles).",
                         ...
                         .remove = function(fileNames, removeLocal){
                           localFiles <- match(fileNames,
                                               basename(private$localAttachments$filePaths))
                           
                           if(any(fileNames[is.na(localFiles)] %in% private$attachmentsToRemove)){
                             stop_custom("error_file_not_found",
                                         "Some of the files to remove do not exist", call. = FALSE)
                           }
