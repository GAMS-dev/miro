HcubeResults <- R6Class("HcubeResults",
  public = list(
    initialize = function(db, gdxio, filePath, outputAttachments, jobMeta,
      ...
      if (any(invalidFiles)) {
        flog.error(
          "HcubeResults: The Hypercube results zip file contains invalid files: %s",
          paste(fileNamesZip[invalidFiles], collapse = ", ")
        )
        stop_custom("error_invalid_results", "Invalid Hypercube results file", call. = FALSE)
      }
      hashesInZip <- sort(unique(unique(vapply(
        strsplit(fileNamesZip, "/"),
        "[[", character(1L), 1L
      ))))
    ...
    extractArchive = function(session, workDir, progressCallback = NULL,
      ...
      if (dir.exists(private$exdir) && !identical(unlink(private$exdir, ), 0L)) {
        flog.error(
          "HcubeResults: Problems removing existing temporary directory: %s",
          private$exdir
        )
      ...
      if (!dir.create(private$exdir, recursive = TRUE)) {
        flog.error(
          "HcubeResults: Problems creating temporary directory to extract Hypercube results into: %s",
          private$exdir
        )
        stop_custom("error_extraction", "Failed extracting Hypercube results archive.", call. = FALSE)
      }
      ...
      tryCatch(
        {
          ...
          i <- 1L
          scenFiles <- c(
            "_scenTrc.trc", private$outputAttachments,
            MIROGdxOutName
          )
          for (scenIdx in seq_len(numberScen)) {
            scenHash <- private$scenHashes[[scenIdx]]
            scenFilesExist <- setNames(
              file.exists(file.path(private$exdir, scenHash, scenFiles)),
              scenFiles
            )
            ...
            private$db$exportScenDataset(
              bind_cols(
                `_sid` = rep.int(firstScenId + scenIdx - 1L, nrow(hcScalarsTmp)),
                hcScalarsTmp
              ),
              "_scalars",
              isHcJobConfig = FALSE
            ...
            if (private$includeTrc) {
              if (scenFilesExist[["_scenTrc.trc"]]) {
                scenData <- readTraceData(file.path(
                  private$exdir, scenHash,
                  "_scenTrc.trc"
                ))[1, ]
                private$db$exportScenDataset(
            ...
            if (scenFilesExist[[MIROGdxOutName]]) {
              for (symName in names(ioConfig$modelOut)) {
                if (identical(symName, scalarsOutName)) {
                  dbSchemaTmp <- list(
                    colNames = c("scalar", "description", "value"),
                    colTypes = "ccc"
                  )
                } else {
                ...
                scenData <- tryCatch(
                  {
                    fixColTypes(
                      private$gdxio$rgdx(gdxOutPath, symName,
            ...
            for (outputAttachment in private$outputAttachments) {
              if (scenFilesExist[[outputAttachment]]) {
                ...
                if (fileSize <= attachMaxFileSize) {
                  ...
                  flog.warn(
                    "HcubeResults: Could not store output attachment: '%s' in database as it exceeds the maximum file size of %s bytes.",
          ...
          if (numberScen > 0L) {
            private$db$exportScenDataset(
              tibble(
                `_sid` = seq(firstScenId, firstScenId + numberScen - 1L),
