ScriptOutput <- R6Class("ScriptOutput", public = list(
  initialize = function(session, workDir, config, errorMsg, gamsSysDir) {
    ...
    if (!identical(.Platform$OS.type, "windows") &&
      ...
      private$scriptEnv[["PATH"]] <- paste0(
        gamsSysDir, .Platform$path.sep,
  ...
  getResults = function(scriptId = NULL) {
    if (is.null(scriptId)) {
      if (length(private$scriptResults)) {
        return(tibble(
          id = names(private$scriptResults),
  ...
  loadResultsBase = function(data, scenId = NULL) {
    ...
    if (is.null(scenId)) {
      ...
      if (any(is.na(orphanedScriptResults))) {
        flog.info(
          paste0(
            "The scenario you loaded includes orphaned script results.",
  ...
  interrupt = function(scriptId = NULL, suppressWarning = FALSE) {
    ...
    if (!suppressWarning) {
      flog.warn(
        "Script: '%s' could not be interrupted as it was not found in list of active scripts.",
        scriptId
      )
    }
    return(invisible(self))
    ...
    private$activeScriptsObs[[scriptId]] <- observe({
      if (private$activeScriptsTo[[scriptId]] == 0L) {
        ...
        if (identical(private$pingProcess(scriptId), 0L)) {
          ...
          flog.warn(
            "Script output: '%s' terminated with exit code: '%s'. Stdout/err: '%s'.",
  ...
  readOutput = function(id, scriptId, hcube = FALSE) {
    ...
    return(tryCatch(
      {
        private$scriptResults[[scriptId]] <- read_file(file.path(
          private$workDir,
          outputFile
        ))
        if (parseMarkdown) {
      ...
      error = function(e) {
        flog.error(
          "Problems reading output file of script: '%s'. Error message: '%s'.",
  ...
  killScript = function(scriptId) {
    if (scriptId %in% names(private$activeScripts)) {
      return(tryCatch(
        ...
        error = function(e) {
          flog.error(
            "Problems terminating script: '%s'. Error message: '%s'.",
            conditionMessage(e)
          )
          return(FALSE)
        }
