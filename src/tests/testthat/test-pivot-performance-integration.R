  ...
  testServer(renderMiroPivot,
    {
      ...
      pr$measure("filter", {
        session$setInputs(
          rowIndexList = dataHeaders[-c(2, 6, length(dataHeaders))],
          filterIndexList = c("soName", "ALLYEAR"),
          filter_soName = c(
            "Cost_Flox", "VAR_Comprd", "Time_NPV",
      ...
      pr$measure("pivot", {
        session$setInputs(
          rowIndexList = dataHeaders[-c(2, 4, 6, 9, length(dataHeaders))],
      ...
      pr$measure("aggregate", {
        session$setInputs(
          rowIndexList = dataHeaders[-c(2, 4, 5, 6, 9, length(dataHeaders))],
    ...
    args = list(
      data = data,
      options = list(
        enablePersistentViews = FALSE, input = TRUE,
