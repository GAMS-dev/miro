...
outScalarNames <- c(
  "error_train", "error_test", "error_ratio",
...
outScalarDataVisible <- tibble(
  scalar = outScalarNames[1:3],
  description = outScalarDesc[1:3],
  value = c(
    "79.6135736013082", "951.166205306451",
    "11.9472869044886"
  )
)
outScalarDataFull <- tibble(
  scalar = outScalarNames,
...
dbSchema <<- DbSchema$new(list(
  schema = list(
    ...
    firstdaytraining = list(
      tabName = "firstdaytraining",
      colNames = "firstdaytraining",
      colTypes = "c"
    ),
    lastdaytraining = list(
      tabName = "lastdaytraining",
...
scenData <<- ScenData$new(
  db = db,
  scenDataTemplate = list(
    tibble(
      scalar = c("error_train", "error_test", "error_ratio"),
      description = "",
      value = NA_character_
    ),
    ...
    tibble(
      date = character(), `absolute error train` = numeric(),
...
test_that("Getting all scalars works with data", {
  expect_error(scenData$load(1, refId = "sb", showProgress = FALSE), NA)
  expect_identical(
    scenData$getScalars("sb"),
    tibble(
      scalar = c(inScalarNames, outScalarNames),
      description = c(inScalarDesc, outScalarDesc),
      value = c(
        "2", "99", "CPLEX", "79.6135736013082", "951.166205306451",
        "11.9472869044886", "2016-01-04", "2016-05-24"
      )
    )
  )
...
test_that("includeHiddenScalars argument works when calling ScenData$get", {
  ...
  expect_identical(
    scenData$get("sb", scalarsOutName, includeHiddenScalars = TRUE)[[1]],
    outScalarDataFull
  )
  expect_identical(scenData$get("sb", scalarsOutName)[[1]], outScalarDataVisible)
  expect_identical(
    scenData$get("sb", scalarsOutName, includeHiddenScalars = TRUE, drop = TRUE),
    outScalarDataFull
  )
...
ioConfig <<- list(
  modelOut = list(x = list()),
  inputDsNames = c()
)
dbSchema <<- DbSchema$new(list(schema = list(x = list(
  tabName = "x",
  colNames = c(
    "z", "g", "c", "t", "s",
    "w", "level", "marginal",
  ...
  colTypes = "ccccccddddd"
)), views = list()))
dbSchema$setConn(db$getConn())

scenData <- ScenData$new(
  db = db,
  scenDataTemplate = list(tibble(
    z = character(),
    g = character(),
    c = character(),
    t = character(),
...
test_that("Remapping scenario names works", {
  ...
  if (identical(Sys.getenv("MIRO_DB_TYPE"), "postgres")) {
    DBI::dbExecute(db$getConn(), paste0(
      "DROP TABLE ",
  ...
  DBI::dbWriteTable(db$getConn(), dbSchema$getDbTableName("_scenMeta"),
    tibble(
      `_sid` = seq_len(6L), `_uid` = c("te_de\\%d", "user", "te_de\\%d", "te_de\\%d", "te_de\\%d", "te_de\\%d"),
