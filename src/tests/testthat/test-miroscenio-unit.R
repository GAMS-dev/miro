...
test_that("Writing miroscen files works", {
  ...
  generateMiroScen(file.path(tmpd, "test.miroscen"),
                   tibble::tibble(sid = 1, uid = "conner", name = "test", stime = Sys.time(),
                                        tags = vector2Csv(letters)),
                   list(i = tibble::tibble('1' = c("seattle", "san-diego"), '2' = rep.int(NA_character_, 2L)),
                        ...
                        `_scalars` = tibble::tibble(scalar = c("_gmsopt_a", "_gmspar_b"),
                                                    description = c("bla", "blubb"),
                                                    value = c("1", "2"))),
                   attachmentsDummy, viewsDummy, scenId = NULL)
  expect_true(file.exists(file.path(tmpd, "test.miroscen")))
  expect_identical(zip::zip_list(file.path(tmpd, "test.miroscen"))[[1]],
                   c("metadata.json", "data.gdx", "views.json", "attachments/",
  ...
  generateMiroScen(file.path(tmpd, "test.miroscen"),
                   ...
                   list(i = tibble::tibble('1' = c("seattle", "san-diego"), '2' = rep.int(NA_character_, 2L)),
                        ...
                        `_scalars` = tibble::tibble(scalar = c("_gmsopt_a", "_gmspar_b"),
                                                    description = c("bla", "blubb"),
                                                    value = c("1", "2"))),
                   attachmentsDummy, viewsDummy, scenId = 2L)
...
test_that("Validating miroscen files works", {
  expect_error(validateMiroScen(file.path("..", "data", "bad1.miroscen")), regexp = "missing",
               class = "error_badformat")
  expect_error(validateMiroScen(file.path("..", "data", "bad2.miroscen")))
  expect_error(validateMiroScen(file.path("..", "data", "bad3.miroscen")), regexp = "metadata\\.json",
               class = "error_badformat")
})
...
test_that("Loading MIRO scenario into specified directory works", {
  ...
  expect_output(f <- loadMiroScen(file.path(tmpd, "test.miroscen"), scenDummy, attachmentsDummy, viewsDummy,
                                  c(names(modelIn), "_gmsopt_a"), exdir = file.path(tmpd, "bla")),
                regexp = "ignored")
