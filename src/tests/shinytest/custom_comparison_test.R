app <- AppDriver$new("../../",
  name = "custom_comparison_test", variant = NULL,
...
Sys.sleep(0.5)
expect_true(app$get_js("$('#btBatchCompare').text()==='Compare (Split)'"))
expect_true(app$get_js("$('#btBatchCompare~.dropdown-menu a:eq(0)').text()==='Compare (Pivot)'"))
expect_true(app$get_js("$('#btBatchCompare~.dropdown-menu a:eq(1)').text()==='Compare (Tab)'"))
...
Sys.sleep(0.5)
app$wait_for_js("($('#shiny-modal').data('bs.modal')||{}).isShown===true", timeout = 5000L)
expect_error(app$wait_for_js("$('#btLoadScenConfirm').is(':visible')", timeout = 50L), NA)
