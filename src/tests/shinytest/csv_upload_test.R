app <- AppDriver$new("../../",
  name = paste0("csv_upload_test_", Sys.getenv("GMSMODELNAME")), variant = NULL,
...
if (identical(Sys.getenv("GMSMODELNAME"), "pickstock")) {
  ...
  expect_match(app$get_text(selector = "#localDataImportError"), "same column", fixed = TRUE)
  ...
  Sys.sleep(1)
  expect_error(app$wait_for_js("$('#localDataImportError').is(':visible')", timeout = 50), NA)
  expect_match(app$get_text(selector = "#localDataImportError"), "could not be identified uniquely", fixed = TRUE)
  app$upload_file(localInput = paste0("../data/csvtest-bad.csv"))
  Sys.sleep(1)
  expect_error(app$wait_for_js("$('#localDataImportError').is(':visible')", timeout = 50), NA)
  expect_match(app$get_text(selector = "#localDataImportError"), "is not UTF-8 encoded", fixed = TRUE)
  ...
  Sys.sleep(0.5)
  expect_error(app$wait_for_js("$('#localDataImportError').is(':visible')", timeout = 50), NA)
  expect_match(app$get_text(selector = "#localDataImportError"), "at least one column to import", fixed = TRUE)
  app$upload_file(localInput = paste0("../data/price.psv"))
  Sys.sleep(1)
