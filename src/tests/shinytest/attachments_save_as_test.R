app <- AppDriver$new("../../", name = "attachments_save_as_test", variant = NULL, load_timeout = 20000)

app$expect_values(input = "inputDataTitle")

saveNewDefault <- function(newName = NULL, discardAttach = FALSE) {
  app$click(selector = "#btRemove1")
  Sys.sleep(0.5)
  app$click(selector = ".modal-footer .bt-gms-confirm")
  Sys.sleep(0.5)
  app$set_inputs(btImport = "click")
  Sys.sleep(0.5)
  app$set_inputs(selLoadScen = paste0("1_", user))
  expect_identical(startsWith(app$get_value(input = "selLoadScen"), "1_"), TRUE)
  app$set_inputs(btLoadScenConfirm = "click")
  Sys.sleep(1)
  app$set_inputs(btSaveAs = "click")
  Sys.sleep(0.5)
  app$set_inputs(scenName = newName)
  if (isTRUE(discardAttach)) {
    app$set_inputs(newScenDiscardAttach = "click")
  }
  Sys.sleep(0.1)
  app$click(selector = "#shiny-modal #dialogSaveInit .bt-gms-confirm")
  Sys.sleep(1)
}


# load default scenario
app$click(selector = "#btRemove1")
Sys.sleep(0.5)
app$click(selector = ".modal-footer .bt-gms-confirm")
Sys.sleep(0.5)
app$set_inputs(btImport = "click")
Sys.sleep(0.5)
app$set_inputs(btLoadScenConfirm = "click")
Sys.sleep(1)

# get user name
app$set_inputs(btEditMeta = "click")
Sys.sleep(0.5)
app$click(selector = "a[data-value='accessPerm']")
Sys.sleep(1)
user <- app$get_value(input = "editMetaWritePerm")[[1]]
app$set_inputs(btUpdateMeta = "click")
Sys.sleep(0.5)

# save as same name (overwrite existing)
app$set_inputs(btSaveAs = "click")
Sys.sleep(0.5)
expect_true(app$get_js("$('#scenarioExists').css('display')==='none'"))
app$click(selector = "#shiny-modal #dialogSaveInit .bt-gms-confirm")
Sys.sleep(0.5)
expect_true(app$get_js("$('#scenarioExists').attr('style')===''"))
app$set_inputs(btSaveConfirm = "click")
Sys.sleep(1)

# save as same name (chose new scenario name)
app$set_inputs(btSaveAs = "click")
Sys.sleep(0.5)
expect_true(app$get_js("$('#scenarioExists').css('display')==='none'"))
app$click(selector = "#shiny-modal #dialogSaveInit .bt-gms-confirm")
Sys.sleep(0.5)
expect_true(app$get_js("$('#scenarioExists').attr('style')===''"))
app$set_inputs(btNewName = "click")
Sys.sleep(0.5)
app$set_inputs(scenName = "newScen")
Sys.sleep(0.5)
app$click(selector = "#shiny-modal #dialogSaveInit .bt-gms-confirm")
Sys.sleep(1)

# add attachment, save scenario newScen
app$set_inputs(btEditMeta = "click")
Sys.sleep(0.5)
app$click(selector = "a[data-value='attachments']")
app$upload_file(file_addAttachments = "../model/pickstock_with_data/README.md")
Sys.sleep(1)
app$set_inputs(btUpdateMeta = "click")
Sys.sleep(1)
app$set_inputs(btSave = "click")
Sys.sleep(0.5)
app$set_inputs(btSaveOutput = "click")
Sys.sleep(1)

# save as same name and discard attachments
app$set_inputs(btSaveAs = "click")
Sys.sleep(0.5)
app$set_inputs(newScenDiscardAttach = "click")
Sys.sleep(0.5)
app$click(selector = "#shiny-modal #dialogSaveInit .bt-gms-confirm")
Sys.sleep(0.5)
app$set_inputs(btSaveConfirm = "click")
Sys.sleep(1)
app$set_inputs(btEditMeta = "click")
Sys.sleep(0.5)
app$click(selector = "a[data-value='attachments']")
Sys.sleep(0.1)
attachmentList <- app$get_js("$('.attachment-line').length")
expect_identical(attachmentList, 0L)
app$set_inputs(btUpdateMeta = "click")
Sys.sleep(0.5)

# add attachment, save as new name and discard attachments
saveNewDefault("newScen2", TRUE)
app$set_inputs(btEditMeta = "click")
Sys.sleep(0.5)
app$click(selector = "a[data-value='attachments']")
Sys.sleep(0.1)
app$upload_file(file_addAttachments = "../model/pickstock_with_data/README.md")
Sys.sleep(1)
app$set_inputs(btUpdateMeta = "click")
Sys.sleep(1)
app$set_inputs(btSave = "click")
Sys.sleep(0.5)
app$set_inputs(btSaveOutput = "click")
Sys.sleep(1)
app$set_inputs(btSaveAs = "click")
Sys.sleep(0.5)
app$set_inputs(scenName = "newScen3")
app$set_inputs(newScenDiscardAttach = "click")
Sys.sleep(0.5)
app$click(selector = "#shiny-modal #dialogSaveInit .bt-gms-confirm")
Sys.sleep(1)
app$set_inputs(btEditMeta = "click")
Sys.sleep(0.5)
app$click(selector = "a[data-value='attachments']")
Sys.sleep(0.5)
attachmentList <- app$get_js("$('.attachment-line').length")
expect_identical(attachmentList, 0L)
app$set_inputs(btUpdateMeta = "click")
Sys.sleep(0.5)

# add attachment, save as same name and do not discard attachments
saveNewDefault("newScen4", TRUE)
app$set_inputs(btEditMeta = "click")
Sys.sleep(0.5)
app$click(selector = "a[data-value='attachments']")
Sys.sleep(0.1)
app$upload_file(file_addAttachments = "../model/pickstock_with_data/README.md")
Sys.sleep(1)
app$set_inputs(btUpdateMeta = "click")
Sys.sleep(1)
app$set_inputs(btSave = "click")
Sys.sleep(0.5)
app$set_inputs(btSaveOutput = "click")
Sys.sleep(1)
app$set_inputs(btSaveAs = "click")
Sys.sleep(0.5)
app$click(selector = "#shiny-modal #dialogSaveInit .bt-gms-confirm")
Sys.sleep(0.5)
app$set_inputs(btSaveConfirm = "click")
Sys.sleep(1)
app$set_inputs(btEditMeta = "click")
Sys.sleep(0.5)
app$click(selector = "a[data-value='attachments']")
Sys.sleep(0.1)
attachmentList <- app$get_js("$('.attachment-line').length")
expect_identical(attachmentList, 1L)
app$set_inputs(btUpdateMeta = "click")
Sys.sleep(0.5)

# add attachment, save as new name and do not discard attachments
saveNewDefault("newScen5", TRUE)
app$set_inputs(btEditMeta = "click")
Sys.sleep(0.5)
app$click(selector = "a[data-value='attachments']")
Sys.sleep(0.1)
app$upload_file(file_addAttachments = "../model/pickstock_with_data/README.md")
Sys.sleep(1)
app$set_inputs(btUpdateMeta = "click")
Sys.sleep(1)
app$set_inputs(btSave = "click")
Sys.sleep(0.5)
app$set_inputs(btSaveOutput = "click")
Sys.sleep(1)
app$set_inputs(btSaveAs = "click")
Sys.sleep(0.5)
app$set_inputs(scenName = "newScen6")
Sys.sleep(0.5)
app$click(selector = "#shiny-modal #dialogSaveInit .bt-gms-confirm")
Sys.sleep(1)
app$set_inputs(btEditMeta = "click")
Sys.sleep(0.5)
app$click(selector = "a[data-value='attachments']")
Sys.sleep(0.1)
attachmentList <- app$get_js("$('.attachment-line').length")
expect_identical(attachmentList, 1L)
app$set_inputs(btUpdateMeta = "click")
Sys.sleep(0.5)

app$stop()
