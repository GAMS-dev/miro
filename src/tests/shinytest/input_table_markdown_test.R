...
expect_true(app$get_js("$('#tableLabel_1').html().includes('<h3>Data:</h3>') && $('#tableLabel_1').html().includes('<p><em>test</em> <strong>asd</strong></p>') && $('#tableLabel_1').html().includes('<mrow><msubsup><mo>') && $('#tableLabel_1').html().includes('5<span>$</span>6<span>$</span>');", timeout = 50))
