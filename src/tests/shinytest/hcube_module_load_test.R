...
app$set_inputs(btSendQuery = "click")
Sys.sleep(2)
expect_true(app$get_js("$('#batchLoadResults').data('datatable').data().length===5;", timeout = 50L))
...
app$set_inputs(btSendQuery = "click")
expect_true(app$get_js("$('#batchLoadResults').data('datatable').data().length===1;", timeout = 50L))
