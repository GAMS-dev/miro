...
Sys.sleep(1)
app$findElement(".btSolve .dropdown-toggle")$click()
app$findElement(".change-dd-button[data-action-id='btSubmitJob']")$click()
Sys.sleep(0.5)
expect_error(app$findElement("#shiny-modal .btn-default")$click(), NA)
...
expect_true(app$waitFor("$('#remoteLoginHostNotFound').is(':visible');", timeout = 5000))
Sys.sleep(2)
app$setInputs(remoteCredUrl = Sys.getenv("ENGINE_URL"))
app$setInputs(remoteCredUser = paste0(Sys.getenv("ENGINE_USER_INVITEE"), "s"))
app$findElement("#shiny-modal .bt-gms-confirm")$click()
expect_true(app$waitFor("$('#remoteLoginInvalidCred').is(':visible');", timeout = 5000))
Sys.sleep(2)
app$setInputs(remoteCredUser = Sys.getenv("ENGINE_USER_INVITEE"))
...
app$findElement("#shiny-modal .bt-gms-confirm")$click()
expect_true(app$waitFor("$('#remoteLoginNsNotFound').is(':visible');", timeout = 5000))
Sys.sleep(2)
# check that model is registered (it's not) and try to solve
app$setInputs(remoteCredNs = Sys.getenv("ENGINE_NS"))
app$setInputs(remoteCredReg = TRUE)
app$findElement("#shiny-modal .bt-gms-confirm")$click()
expect_true(app$waitFor("$('#remoteLoginModelNotFound').is(':visible');", timeout = 5000))
Sys.sleep(2)
...
app$findElement("#refreshActiveJobs")$click()
Sys.sleep(1)
app$waitFor("$(\"#jImport_output button[onclick*='downloadJobData']\").is(':visible')", timeout = 10000)
app$snapshot()
app$findElements("#jImport_output button[onclick*='downloadJobData']")[[1]]$click()
Sys.sleep(1)
app$waitFor("$(\"#shiny-modal button[data-dismiss='modal']\").is(':visible')", timeout = 10000)
app$findElement("#shiny-modal button[data-dismiss='modal']")$click()
Sys.sleep(1)
...
app$findElement("#refreshActiveJobs")$click()
Sys.sleep(1)
app$waitFor("$(\"#jImport_output button[onclick*='showJobLog']\").is(':visible')", timeout = 10000)
app$findElements("#jImport_output button[onclick*='showJobLog']")[[1]]$click()
Sys.sleep(1)
...
app$findElement(".btSolve .dropdown-toggle")$click()
app$findElement(".change-dd-button[data-action-id='btSolve']")$click()
app$waitFor("$('#modelStatus').is(':visible') && $('#modelStatus').text().startsWith('Model execution phase');", timeout = 10000)
app$snapshot()
app$setInputs(btInterrupt = "click")
app$waitFor("$('#modelStatus').is(':visible') && $('#modelStatus').text().startsWith('There was a compilation error');", timeout = 10000)
...
app$findElement(".btSolve .dropdown-toggle")$click()
app$findElement(".change-dd-button[data-action-id='btSolve']")$click()
app$waitFor("$('#modelStatus').is(':visible') && $('#modelStatus').text().startsWith('Quota exceeded');", timeout = 10000)
expect_true(app$waitFor("$('.shiny-notification-content').is(':visible');", timeout = 50L))
...
app$waitFor("$('.shiny-notification-close').click()", timeout = 50L)
...
app$setInputs(jobSubmissionName = "test1")
app$findElement("#btSubmitAsyncJob")$click()
expect_true(app$waitFor("$('#jobSubmitUnknownError').text().includes('quota was exceeded')", timeout = 10000))
expect_true(app$waitFor("($('#shiny-modal').data('bs.modal')||{}).isShown!==true", timeout = 10000L))
expect_true(app$waitFor("$('.shiny-notification-content').text().includes('Quota exceeded');", timeout = 50L))
expect_true(app$waitFor("/volume quota: -?\\\\d+ s/.test($('.shiny-notification-content').text())", timeout = 50L))
app$waitFor("$('.shiny-notification-close').click()", timeout = 50L)
