getScenTabData <- function(sheetName) {
  ...
  if (is.na(i)) {
    ...
    if (is.na(i)) {
      stop(sprintf(
        "Sheet: %s could not be rendered as it was not found in either list of input or output sheets.",
        sheetName
      ), call. = FALSE)
    } else {
    ...
    if (identical(inputDsNames[[i]], scalarsFileName) &&
      ...
      tabData$headerAliases <- c(
        lang$nav$scalarAliases$cols$name,
  ...
  if (is.na(tabData$scenTableId)) {
    stop(sprintf(
      "Data for sheet: '%s' could not be found. If this problem persists, please contact the system administrator.",
      sheetName
    ), call. = FALSE)
  }
  return(tabData)
    ...
    function(groupId) {
      ...
      content <- lapply(tabSheetIds, function(sheetId) {
        ...
        if (is.null(tabContent)) {
          tabContent <- tagList(
            tags$div(
              id = paste0("scenGraph_", scenId, "_", tabId),
              class = "render-output",
            ...
            if (!pivotCompare) {
              tags$div(
                id = paste0("scenTable_", scenId, "_", tabId),
                style = "display:none;",
                class = "render-output"
              )
            }
      ...
      lapply(seq_along(config$scripts$base), function(scriptId) {
        tabPanel(
          ...
          tagList(
            ...
            tags$div(class = "space"),
            tags$div(
              id = paste0("scenScript_", scenId, "_", scriptId, "_noData"),
              class = "out-no-data", lang$nav$outputScreen$boxResults$noData
            ),
            tags$iframe(
              id = paste0("scenScript_", scenId, "_", scriptId),
              class = "script-output"
            ),
            tags$div(class = "space")
          )
...
generateScenarioTabsetMulti <- function(scenId) {
  tryCatch(
    ...
    error = function(e) {
      flog.error(
        "Problems generating scenario tabset (multi comparison mode). Error message: %s.",
        conditionMessage(e)
      )
  ...
  tabPanel(tags$span(id = paste0("cmpScenTitle_", scenId)),
    value = paste0("scen_", scenId, "_"),
    tags$div(
      class = "scen-header",
      tags$div(
        class = "scen-date-wrapper",
        tags$span(id = paste0("cmpScenDate_", scenId))
      ),
      tags$div(
        class = "scen-buttons-wrapper",
        tags$div(
          title = lang$nav$scen$tooltips$btRefresh, class = "scen-button-tt",
          id = paste0("refreshSandbox_", scenId),
          tags$button(
            class = "btn btn-default scen-button",
            type = "button",
            onclick = paste0(
              "Shiny.setInputValue('btRefreshComp',",
        ...
        tags$div(
          title = lang$nav$scen$tooltips$btExport, class = "scen-button-tt",
          HTML(paste0(
            '<button type="button" class="btn btn-default scen-button"
onclick="Shiny.setInputValue(\'btExportScen\', ', scenId, ', {priority: \'event\'})"><i class="fas fa-download" role="presentation" aria-label="',
        ...
        tags$div(
          ...
          tags$button(
            ...
            onclick = paste0(
              "Shiny.setInputValue('btScenTableView',",
              scenId, ",{priority:'event'})"
            ),
            tags$i(
...
generateScenarioTabsetSplit <- function(scenId) {
  tryCatch(
    ...
    error = function(e) {
      flog.error(
        "Problems generating scenario tabset (split comparison mode). Error message: %s.",
        conditionMessage(e)
      )
      stop(conditionMessage(e))
    }
    ...
    tags$div(
      ...
      box(
        ...
        width = 12L, solidHeader = TRUE, status = "primary", title =
          tagList(
            tags$button(
              title = lang$nav$scen$tooltips$btAddPivot,
        ...
        tags$div(
          id = "pivotCompBtWrapper", class = "no-scen", lang$nav$scen$noScen,
          tags$div(
            style = "margin: 10px;",
            tags$button(
