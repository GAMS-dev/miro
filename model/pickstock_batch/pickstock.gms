...
$if not set maxstock $set maxstock 2
$if not set trainingdays $set trainingdays 100
...
Scalar maxstock        'maximum number of stocks to select'  / %maxstock%  /
       trainingdays    'number of days for training       '  / %trainingdays%  /;
       
$if not set fileName $set fileName dowjones2016.csv
$ifthen exist "%gams.idir1%%fileName%"
$  call csv2gdx "%gams.idir1%%fileName%" output=stockdata.gdx ValueDim=0 id=price Index=(1,2) Value=3 UseHeader=y
