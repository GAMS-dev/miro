...
LINUX:
    ...
    g++ read_psse32.cpp $GAMSCPPAPI/gdxco.cpp $GAMSCAPI/gdxcc.c -I$GAMSCPPAPI -I$GAMSCAPI -std=c++0x -ldl -o read_psse32
Windows (cmd):
    cl /EHsc read_psse32.cpp %GAMSDIR%\apifiles\C++\api\gdxco.cpp %GAMSDIR%\apifiles\C\api\gdxcc.c -I %GAMSDIR%\apifiles\C++\api -I %GAMSDIR%\apifilatof(busnum[i]), 2 );
Windows (PowerShell):
    cl /EHsc read_psse32.cpp $env:GAMSDIR\apifiles\C++\api\gdxco.cpp $env:GAMSDIR\apifiles\C\api\gdxcc.c -I $env:GAMSDIR\apifiles\C++\api -I $env:GAMSDIR\apifiles\C\api
*/
...
//        owner[numbuses] = atoi(cell);
    ...
    gshunt = new double[numbuses];
    bshunt = new double[numbuses];
    for( j=0; j<numbuses; j++ )
    {
        gshunt[j] = 0;
        bshunt[j] = 0;
    ...
    cout << "Getting fixed shunt data" << endl;
    // Get load data
    while( LINE.find("BEGIN FIXED SHUNT DATA") == string::npos )
    {
        getline(infile, LINE);
    }
    //cout << LINE << endl;
    ...
    strcpy(line, LINE.c_str());
    cell = strtok(line," ,");
    while( LINE.find("END OF FIXED SHUNT DATA") == string::npos )
    {
        //cout << LINE << endl;
        // The first entry can be either bus number or bus name, but in
        // all of our data it's bus number, so we're going with that.
    ...
    while( LINE.find("END OF SWITCHED SHUNT DATA") == string::npos )
    {
        ...
        // Ignore the shunt type, voltage, and name parameters
        cell = strtok(NULL, " ,'");
        cell = strtok(NULL, " ,'");
        cell = strtok(NULL, " ,'");
        cell = strtok(NULL, " ,'");
