...
$if %quitearly%==1 $exit
execute 'gams "raw2gdx.gms" idir="%filepath%" lo=3 gdxcompress=1 --raw="%out%raw" gdx="%out%" --monitorall=%monitorall% profile=1 stepsum=1'
if(errorlevel ne 0, abort "raw2gdx failed!");
execute 'gams "save_domain_info.gms" idir="%filepath%" gdx="%out%" --in="%out%raw"'
if(errorlevel ne 0, abort "Save domain info failed!");
$if %quitearly%==2 $exit
execute 'rm "%out%raw.gdx"'

$ifthen (%Sbus%==1 and %verbose% == 1)
execute 'gams "calc_S_matrix.gms" idir="%filepath%" lo=3 gdxcompress=1 --case="%out%"'
$elseif %Sbus%==1
execute 'gams "calc_S_matrix.gms" idir="%filepath%" lo=3 gdxcompress=1 --case="%out%" lo=0'
$endif
