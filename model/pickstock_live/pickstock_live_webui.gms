...
$onmulti
$if setenv PICKSTOCK_LIVE_MAXSTOCK Scalar maxstock / %sysEnv.PICKSTOCK_LIVE_MAXSTOCK% /
$if setenv PICKSTOCK_LIVE_TRAININGDAYS Scalar trainingdays / %sysEnv.PICKSTOCK_LIVE_TRAININGDAYS% /
$offmulti
$ondigit
$onuni
...
webuis1_(webui0_) = sum(webui1_, price(webui0_,webui1_));
price(webuis1_,symbol) = price(webuis1_,symbol) + eps;
...
writeCSVParam(db["price"],"gmswebui.gdx", -1)
writeCSVParam(db["partOfPortfolio"],"gmswebui.gdx", -1)
writeCSVParam(db["dowVSindex"],"gmswebui.gdx", -1)
writeCSVParam(db["abserror"],"gmswebui.gdx", -1)
