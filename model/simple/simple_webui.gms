$onExternalInput
$kill planttypedata timeseries
*$killUel
$offExternalInput
$offdigit
$batInclude loadCSV planttypedata
$batInclude loadCSV timeseries
$onmulti
$offmulti
$ondigit
$onuni
...
webuis2_(webui0_,webui1_) = sum(webui2_, rep_emix(webui0_,webui1_,webui2_));
rep_emix(webuis2_,emixHdr) = rep_emix(webuis2_,emixHdr) + eps;
option clear=webuis2_;
webuis3_(webui0_,webui1_,webui2_) = sum(webui3_, rep_flow(webui0_,webui1_,webui2_,webui3_));
rep_flow(webuis3_,flowHdr) = rep_flow(webuis3_,flowHdr) + eps;
...
writeCSVParam(db["rep_emix"],"gmswebui.gdx")
writeCSVParam(db["rep_flow"],"gmswebui.gdx")
with open('scalars_out.csv', 'w') as f:
   f.write('Scalar,Description,Value\n')
   f.write('obj_rep,"objective function value",' + str(db['obj_rep'].first_record().value) + '\n')
   f.closed
db.__del__()
endEmbeddedCode
