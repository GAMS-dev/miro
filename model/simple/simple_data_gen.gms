...
*display rLocData;
distance(rr1,rr2)  = edist(rLocData(rr1,'x')-rLocData(rr2,'x'),rLocData(rr1,'y')-rLocData(rr2,'y'));
...
plantsize(rp(rr,p))   = 0.5+rLocData(rr,'pop')/2;
...
demandX(rr,ttX)                    = regionsize(rr) * 0.001 * timeseries(ttX,'demand') * uniform(0.9,1.0);
*ecarSum                            = sum(ttX,0.001 * timeseries(ttX,'ecars'));
demandSum(rr)                      = sum(ttX, demandX(rr,ttX))*uniform(0.05,0.15);
*ecarNorm(ttX)                      = 0.001 * timeseries(ttX,'ecars')/ecarSum;
*demandEcarX(rr,ttX)                = ecarNorm(ttX)*demandSum(rr)*uniform(0.95,1.05);

*renewablesX(rr,ttX)                = regionsize(rr) * (0.001 * timeseries(ttX,'wind') + 0.001 * timeseries(ttX,'photovoltaic')) * uniform(0.9,1.1);
* populate parameters without time index
yearly_plant_cap(rp)               = sum(ttX, plant_capX(rp,ttX)) * uniform(0.9,0.96);
plant_emission(rp,'CO2')           = sum(ptype(rp,type), planttypedata(type,'CO2')) * uniform(0.9,1.1);
cost_power_generation(rp)          = sum(ptype(rp,type), planttypedata(type,'cost')) * uniform(0.9,1.1);
...
cost_plant_add(rp)                 = sum(ptype(rp,type), planttypedata(type,'Expansion Cost')) * uniform(0.9,1.1);;
